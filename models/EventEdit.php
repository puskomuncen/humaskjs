...
class EventEdit extends Event
{
    ...
    // Page object name
    public string $PageObjName = "EventEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "eventedit";
...
// Is modal
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("event_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->event_id->setQueryStringValue($keyValue);
                $this->event_id->setOldValue($this->event_id->QueryStringValue);
            } elseif (Post("event_id") !== null) {
                $this->event_id->setFormValue(Post("event_id"));
                $this->event_id->setOldValue($this->event_id->FormValue);
            } else {
...
// Get upload files
    ...
    protected function loadFormValues(): void
    {
        ...
        // Check field name 'event_id' before field var 'x_event_id'
        $val = $this->getFormValue("event_id", null) ?? $this->getFormValue("x_event_id", null);
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            if ($this->event_id->Visible && $this->event_id->Required) {
                if (!$this->event_id->IsDetailKey && IsEmpty($this->event_id->FormValue)) {
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // nama_event
        $this->nama_event->setDbValueDef($newRow, $this->nama_event->CurrentValue, $this->nama_event->ReadOnly);
        ...
        // lokasi
        $this->lokasi->setDbValueDef($newRow, $this->lokasi->CurrentValue, $this->lokasi->ReadOnly);

        // mitra_id
        $this->mitra_id->setDbValueDef($newRow, $this->mitra_id->CurrentValue, $this->mitra_id->ReadOnly);

        // peserta_terdaftar
        $this->peserta_terdaftar->setDbValueDef($newRow, $this->peserta_terdaftar->CurrentValue, $this->peserta_terdaftar->ReadOnly);
        return $newRow;
    }
    ...
    {
        if (isset($row['nama_event'])) { // nama_event
            $this->nama_event->CurrentValue = $row['nama_event'];
        }
        ...
        if (isset($row['peserta_terdaftar'])) { // peserta_terdaftar
            $this->peserta_terdaftar->CurrentValue = $row['peserta_terdaftar'];
        }
    }
