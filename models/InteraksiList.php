 ...
 * Page class
 */
class InteraksiList extends Interaksi
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "InteraksiList";
    ...
    public string $FormName = "finteraksilist";

    // CSS class/style
    public string $CurrentPageName = "interaksilist";
    ...
    {
        $this->interaksi_id->setVisibility();
        $this->mitra_id->setVisibility();
        $this->user_id->setVisibility();
        $this->jenis_interaksi->setVisibility();
        $this->catatan->setVisibility();
        $this->tanggal_interaksi->setVisibility();
    }
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        // Table object (interaksi)
        if (!isset($GLOBALS["interaksi"]) || $GLOBALS["interaksi"]::class == PROJECT_NAMESPACE . "interaksi") {
            $GLOBALS["interaksi"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "interaksiadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "interaksidelete";
        $this->MultiUpdateUrl = "interaksiupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'interaksi');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "interaksiview"); // If View page, no primary button
                } else { // List page
    ...
    protected function hideFieldsForAddEdit(): void
    {
        if ($this->isAdd() || $this->isCopy() || $this->isGridAdd()) {
            $this->interaksi_id->Visible = false;
        }
    }
...
// Is modal
        ...
        // Set up lookup cache
        $this->setupLookupOptions($this->jenis_interaksi);

        // Update form name to avoid conflict
        if ($this->IsModal) {
            $this->FormName = "finteraksigrid";
        }
		...
		if (ReadCookie('interaksi_searchpanel') == 'notactive' || ReadCookie('interaksi_searchpanel') == "") {
			RemoveClass($this->SearchPanelClass, "show");
			AppendClass($this->SearchPanelClass, "collapse");
		} elseif (ReadCookie('interaksi_searchpanel') == 'active') {
    ...
    public function getFilterList(): string
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("finteraksisrch");
        }
        $filterList = Concat($filterList, $this->interaksi_id->AdvancedSearch->toJson(), ","); // Field interaksi_id
        ...
        $filterList = Concat($filterList, $this->jenis_interaksi->AdvancedSearch->toJson(), ","); // Field jenis_interaksi
        $filterList = Concat($filterList, $this->catatan->AdvancedSearch->toJson(), ","); // Field catatan
        $filterList = Concat($filterList, $this->tanggal_interaksi->AdvancedSearch->toJson(), ","); // Field tanggal_interaksi
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("finteraksisrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        // Field interaksi_id
        $this->interaksi_id->AdvancedSearch->SearchValue = $filter["x_interaksi_id"] ?? "";
        $this->interaksi_id->AdvancedSearch->SearchOperator = $filter["z_interaksi_id"] ?? "";
        $this->interaksi_id->AdvancedSearch->SearchCondition = $filter["v_interaksi_id"] ?? "";
        $this->interaksi_id->AdvancedSearch->SearchValue2 = $filter["y_interaksi_id"] ?? "";
        $this->interaksi_id->AdvancedSearch->SearchOperator2 = $filter["w_interaksi_id"] ?? "";
        $this->interaksi_id->AdvancedSearch->save();
        ...
        // Field jenis_interaksi
        $this->jenis_interaksi->AdvancedSearch->SearchValue = $filter["x_jenis_interaksi"] ?? "";
        $this->jenis_interaksi->AdvancedSearch->SearchOperator = $filter["z_jenis_interaksi"] ?? "";
        $this->jenis_interaksi->AdvancedSearch->SearchCondition = $filter["v_jenis_interaksi"] ?? "";
        $this->jenis_interaksi->AdvancedSearch->SearchValue2 = $filter["y_jenis_interaksi"] ?? "";
        $this->jenis_interaksi->AdvancedSearch->SearchOperator2 = $filter["w_jenis_interaksi"] ?? "";
        $this->jenis_interaksi->AdvancedSearch->save();
        ...
        // Field tanggal_interaksi
        $this->tanggal_interaksi->AdvancedSearch->SearchValue = $filter["x_tanggal_interaksi"] ?? "";
        $this->tanggal_interaksi->AdvancedSearch->SearchOperator = $filter["z_tanggal_interaksi"] ?? "";
        $this->tanggal_interaksi->AdvancedSearch->SearchCondition = $filter["v_tanggal_interaksi"] ?? "";
        $this->tanggal_interaksi->AdvancedSearch->SearchValue2 = $filter["y_tanggal_interaksi"] ?? "";
        $this->tanggal_interaksi->AdvancedSearch->SearchOperator2 = $filter["w_tanggal_interaksi"] ?? "";
        $this->tanggal_interaksi->AdvancedSearch->save();
    ...
    protected function setupSortOrder(): void
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->interaksi_id); // interaksi_id
            $this->updateSort($this->mitra_id); // mitra_id
            $this->updateSort($this->user_id); // user_id
            $this->updateSort($this->jenis_interaksi); // jenis_interaksi
            $this->updateSort($this->catatan); // catatan
            $this->updateSort($this->tanggal_interaksi); // tanggal_interaksi
    ...
    protected function resetCmd(): void
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->interaksi_id->setSort("");
                $this->mitra_id->setSort("");
                $this->user_id->setSort("");
                $this->jenis_interaksi->setSort("");
                $this->catatan->setSort("");
                $this->tanggal_interaksi->setSort("");
            }
    ...
    public function renderListOptions(): void
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($this->security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"interaksi\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $this->language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($this->security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"interaksi\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-ask=\"1\" data-btn=\"SaveBtn\">" . $this->language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($this->security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"interaksi\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("CopyLink") . "</a>";
                } else {
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"finteraksilist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"finteraksilist\" data-key=\"" . $this->keyToJson(true) .
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->interaksi_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
			$item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"interaksi\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "interaksi_id");
            $this->createColumnOption($option, "mitra_id");
            $this->createColumnOption($option, "user_id");
            $this->createColumnOption($option, "jenis_interaksi");
            $this->createColumnOption($option, "catatan");
            $this->createColumnOption($option, "tanggal_interaksi");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"finteraksisrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"finteraksisrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="finteraksilist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    public function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_interaksi", "data-rowtype" => RowType::ADD]);
...
// Load basic search values
    ...
    {
        $row = [];
        $row['interaksi_id'] = $this->interaksi_id->DefaultValue;
        $row['mitra_id'] = $this->mitra_id->DefaultValue;
        $row['user_id'] = $this->user_id->DefaultValue;
        $row['jenis_interaksi'] = $this->jenis_interaksi->DefaultValue;
        $row['catatan'] = $this->catatan->DefaultValue;
        $row['tanggal_interaksi'] = $this->tanggal_interaksi->DefaultValue;
        return $row;
    }
	...
	// Begin of add Search Panel Status by Masino Sinaga, October 13, 2024
		...
		if (ReadCookie('interaksi_searchpanel') == 'notactive' || ReadCookie('interaksi_searchpanel') == "") {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"finteraksisrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		} elseif (ReadCookie('interaksi_searchpanel') == 'active') {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle active\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"finteraksisrch\" aria-pressed=\"true\">" . $this->language->phrase("SearchLink") . "</a>";
		} else {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"finteraksisrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		}
        $item->Visible = true;
