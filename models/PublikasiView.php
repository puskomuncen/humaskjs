 ...
 * Page class
 */
class PublikasiView extends Publikasi
{
    use MessagesTrait;
    ...
    // Page object name
    public string $PageObjName = "PublikasiView";
    ...
    // CSS class/style
    public string $CurrentPageName = "publikasiview";
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        if (($keyValue = Get("publikasi_id") ?? Route("publikasi_id")) !== null) {
            $this->RecKey["publikasi_id"] = $keyValue;
        }
...
// Is modal
        ...
        if (($keyValue = Get("publikasi_id") ?? Route("publikasi_id")) !== null) {
            ...
            $this->RecKey["publikasi_id"] = $this->publikasi_id->QueryStringValue;
        } elseif (Post("publikasi_id") !== null) {
            $this->publikasi_id->setFormValue(Post("publikasi_id"));
            $this->RecKey["publikasi_id"] = $this->publikasi_id->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->publikasi_id->setQueryStringValue($keyValue);
            $this->RecKey["publikasi_id"] = $this->publikasi_id->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "publikasilist"; // Return to list
        }
            ...
            case "show": // Get a record to display
                if (!$this->IsModal && !IsApi()) { // Normal view page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("publikasilist"); // Return to list page
                        return;
                        ...
                        while ($this->fetch()) {
                            if (SameString($this->publikasi_id->CurrentValue, $this->CurrentRow['publikasi_id'])) {
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "publikasilist"; // No matching record, return to list
                    } else {
