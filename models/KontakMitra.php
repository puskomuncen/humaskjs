...
/**
 * Table class for kontak_mitra
 */
class KontakMitra extends DbTable implements LookupTableInterface
{
    ...
    // Fields
    public DbField $kontak_id;
    public DbField $mitra_id;
    public DbField $nama_kontak;
    public DbField $jabatan;
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "kontak_mitra"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // kontak_id
        $this->kontak_id = new DbField(
            $this, // Table
            'x_kontak_id', // Variable name
            'kontak_id', // Name
            '`kontak_id`', // Expression
            '`kontak_id`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`kontak_id`', // Virtual expression
            false, // Is virtual
        ...
        $this->kontak_id->InputTextType = "text";
        $this->kontak_id->Raw = true;
        $this->kontak_id->IsAutoIncrement = true; // Autoincrement field
        $this->kontak_id->IsPrimaryKey = true; // Primary key field
        $this->kontak_id->Nullable = false; // NOT NULL field
        $this->kontak_id->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->kontak_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['kontak_id'] = &$this->kontak_id;
        ...
        // nama_kontak
        $this->nama_kontak = new DbField(
            $this, // Table
            'x_nama_kontak', // Variable name
            'nama_kontak', // Name
            '`nama_kontak`', // Expression
            '`nama_kontak`', // Basic search expression
            200, // Type
            100, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`nama_kontak`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->nama_kontak->InputTextType = "text";
        $this->nama_kontak->Nullable = false; // NOT NULL field
        $this->nama_kontak->Required = true; // Required field
        $this->nama_kontak->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        $this->Fields['nama_kontak'] = &$this->nama_kontak;

        // jabatan
        $this->jabatan = new DbField(
            $this, // Table
            'x_jabatan', // Variable name
            'jabatan', // Name
            '`jabatan`', // Expression
            '`jabatan`', // Basic search expression
            200, // Type
            50, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`jabatan`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->jabatan->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "kontak_mitra";
    }
    ...
    public function insert(array &$row): int|bool
    {
        ...
        if ($result) {
            $this->kontak_id->setDbValue($conn->lastInsertId());
            $row['kontak_id'] = $this->kontak_id->DbValue;
        }
        return $result;
    }
    ...
    public function update(array $row, string|array $where = "", ?array $old = null, bool $currentFilter = true): int|bool
    {
        ...
        if ($success) {
            if (!isset($row['kontak_id']) && !IsEmpty($this->kontak_id->CurrentValue)) {
                $row['kontak_id'] = $this->kontak_id->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(?array $row, string|array $where = ""): QueryBuilder
    {
        ...
        $where = is_array($where) ? $this->arrayToFilter($where) : $where;
        if ($row) {
            if (array_key_exists('kontak_id', $row)) {
                AddFilter($where, QuotedName('kontak_id', $this->Dbid) . '=' . QuotedValue($row['kontak_id'], $this->kontak_id->DataType, $this->Dbid));
            }
        }
    ...
    protected function loadDbValues(?array $row)
    {
        ...
        $this->kontak_id->DbValue = $row['kontak_id'];
        $this->mitra_id->DbValue = $row['mitra_id'];
        $this->nama_kontak->DbValue = $row['nama_kontak'];
    ...
    {
        $keys = [];
        $val = $current ? $this->kontak_id->CurrentValue : $this->kontak_id->OldValue;
        if (IsEmpty($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey(string $key, bool $current = false, ?string $keySeparator = null): void
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->kontak_id->CurrentValue = $keys[0];
            } else {
                $this->kontak_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter(?array $row = null, bool $current = false): string
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('kontak_id', $row) ? $row['kontak_id'] : null;
        } else {
            $val = !IsEmpty($this->kontak_id->OldValue) && !$current ? $this->kontak_id->OldValue : $this->kontak_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@kontak_id@", AdjustSql($val), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("kontakmitralist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "kontakmitraview" => $this->language->phrase("View"),
            "kontakmitraedit" => $this->language->phrase("Edit"),
            "kontakmitraadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "KontakMitraView",
            Config("API_ADD_ACTION") => "KontakMitraAdd",
            Config("API_EDIT_ACTION") => "KontakMitraEdit",
            Config("API_DELETE_ACTION") => "KontakMitraDelete",
            Config("API_LIST_ACTION") => "KontakMitraList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("kontakmitraview", $parm);
        } else {
            $url = $this->keyUrl("kontakmitraview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("kontakmitraedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("kontakmitralist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("kontakmitraadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("kontakmitralist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("kontakmitradelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"kontak_id\":" . VarToJson($this->kontak_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->kontak_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->kontak_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys(): array
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("kontak_id") ?? Route("kontak_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys(bool $setCurrent = true): string
    {
        ...
        foreach ($arKeys as $key) {
            if ($setCurrent) {
                $this->kontak_id->CurrentValue = $key;
            } else {
                $this->kontak_id->OldValue = $key;
            }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->kontak_id);
                    $doc->exportCaption($this->mitra_id);
                    $doc->exportCaption($this->nama_kontak);
        ...
        while (($row = $result->fetchAssociative()) && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->kontak_id);
                        $doc->exportField($this->mitra_id);
                        $doc->exportField($this->nama_kontak);
