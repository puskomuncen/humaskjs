 ...
 * Page class
 */
class UserlevelsDelete extends Userlevels
{
    use MessagesTrait;
    ...
    // Page object name
    public string $PageObjName = "UserlevelsDelete";
    ...
    // CSS class/style
    public string $CurrentPageName = "userlevelsdelete";
...
// Use layout
        ...
        if ($filter == "") {
            $this->terminate("userlevelslist"); // Prevent SQL injection, return to list
            return;
        }
    ...
    protected function deleteRows(): ?bool
    {
        ...
        foreach ($oldRows as $row) {
            ...
            if ($deleteRow === null) { // Row skipped
                ...
                $successKeys[] = $thisKey;
                if ($x_ID != null) {
                    $conn->executeStatement("DELETE FROM " . Config("USER_LEVEL_PRIV_TABLE") . " WHERE " . Config("USER_LEVEL_PRIV_USER_LEVEL_ID_FIELD") . " = " . $x_ID); // Delete user rights as well
                }
            }
        }
