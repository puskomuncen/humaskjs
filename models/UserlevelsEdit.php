...
class UserlevelsEdit extends Userlevels
{
    ...
    // Page object name
    public string $PageObjName = "UserlevelsEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "userlevelsedit";
...
// Get upload files
    ...
    protected function loadFormValues(): void
    {
        ...
        if ($this->hasFormValue("o_ID")) {
            $this->ID->setOldValue($this->getFormValue("o_ID"));
        }
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if (in_array($this->ID->CurrentValue, [-2, -1, 0])) {
    ...
    protected function editRow(): bool
    {
        ...
        if ($this->ID->CurrentValue != "") {
            $filterChk = "(`ID` = " . AdjustSql($this->ID->CurrentValue) . ")";
        ...
        if ($updateRow) {
            ...
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("UsersGrid");
            if (in_array("users", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // ID
        $this->ID->setDbValueDef($newRow, $this->ID->CurrentValue, $this->ID->ReadOnly);

        // Name
        $this->Name->setDbValueDef($newRow, $this->Name->CurrentValue, $this->Name->ReadOnly);

        // Hierarchy
        $this->Hierarchy->setDbValueDef($newRow, $this->Hierarchy->CurrentValue, $this->Hierarchy->ReadOnly);

        // Level_Origin
        $this->Level_Origin->setDbValueDef($newRow, $this->Level_Origin->CurrentValue, $this->Level_Origin->ReadOnly);
        return $newRow;
    }
    ...
    protected function restoreEditFormFromRow(array $row): void
    {
        ...
        if (isset($row['Hierarchy'])) { // Hierarchy
            $this->Hierarchy->CurrentValue = $row['Hierarchy'];
        }
        if (isset($row['Level_Origin'])) { // Level_Origin
            $this->Level_Origin->CurrentValue = $row['Level_Origin'];
        }
    }
