...
class MitraList extends Mitra
{
    ...
    // Page object name
    public string $PageObjName = "MitraList";
    ...
    public string $FormName = "fmitralist";

    // CSS class/style
    public string $CurrentPageName = "mitralist";
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        // Initialize URLs
        $this->AddUrl = "mitraadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "mitradelete";
        $this->MultiUpdateUrl = "mitraupdate";
...
// Is modal
		...
		if (ReadCookie('mitra_searchpanel') == 'notactive' || ReadCookie('mitra_searchpanel') == "") {
			RemoveClass($this->SearchPanelClass, "show");
			AppendClass($this->SearchPanelClass, "collapse");
		} elseif (ReadCookie('mitra_searchpanel') == 'active') {
    ...
    public function getFilterList(): string
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fmitrasrch");
        }
        $filterList = Concat($filterList, $this->mitra_id->AdvancedSearch->toJson(), ","); // Field mitra_id
        $filterList = Concat($filterList, $this->nama_mitra->AdvancedSearch->toJson(), ","); // Field nama_mitra
        $filterList = Concat($filterList, $this->jenis_mitra->AdvancedSearch->toJson(), ","); // Field jenis_mitra
        ...
        $filterList = Concat($filterList, $this->tanggal_bergabung->AdvancedSearch->toJson(), ","); // Field tanggal_bergabung
        $filterList = Concat($filterList, $this->status_aktif->AdvancedSearch->toJson(), ","); // Field status_aktif
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fmitrasrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        // Field nama_mitra
        $this->nama_mitra->AdvancedSearch->SearchValue = $filter["x_nama_mitra"] ?? "";
        $this->nama_mitra->AdvancedSearch->SearchOperator = $filter["z_nama_mitra"] ?? "";
        $this->nama_mitra->AdvancedSearch->SearchCondition = $filter["v_nama_mitra"] ?? "";
        $this->nama_mitra->AdvancedSearch->SearchValue2 = $filter["y_nama_mitra"] ?? "";
        $this->nama_mitra->AdvancedSearch->SearchOperator2 = $filter["w_nama_mitra"] ?? "";
        $this->nama_mitra->AdvancedSearch->save();

        // Field jenis_mitra
        $this->jenis_mitra->AdvancedSearch->SearchValue = $filter["x_jenis_mitra"] ?? "";
        $this->jenis_mitra->AdvancedSearch->SearchOperator = $filter["z_jenis_mitra"] ?? "";
        $this->jenis_mitra->AdvancedSearch->SearchCondition = $filter["v_jenis_mitra"] ?? "";
        $this->jenis_mitra->AdvancedSearch->SearchValue2 = $filter["y_jenis_mitra"] ?? "";
        $this->jenis_mitra->AdvancedSearch->SearchOperator2 = $filter["w_jenis_mitra"] ?? "";
        $this->jenis_mitra->AdvancedSearch->save();
        ...
        $this->tanggal_bergabung->AdvancedSearch->SearchValue = $filter["x_tanggal_bergabung"] ?? "";
        $this->tanggal_bergabung->AdvancedSearch->SearchOperator = $filter["z_tanggal_bergabung"] ?? "";
        $this->tanggal_bergabung->AdvancedSearch->SearchCondition = $filter["v_tanggal_bergabung"] ?? "";
        $this->tanggal_bergabung->AdvancedSearch->SearchValue2 = $filter["y_tanggal_bergabung"] ?? "";
        $this->tanggal_bergabung->AdvancedSearch->SearchOperator2 = $filter["w_tanggal_bergabung"] ?? "";
        $this->tanggal_bergabung->AdvancedSearch->save();

        // Field status_aktif
        $this->status_aktif->AdvancedSearch->SearchValue = $filter["x_status_aktif"] ?? "";
        $this->status_aktif->AdvancedSearch->SearchOperator = $filter["z_status_aktif"] ?? "";
        $this->status_aktif->AdvancedSearch->SearchCondition = $filter["v_status_aktif"] ?? "";
        $this->status_aktif->AdvancedSearch->SearchValue2 = $filter["y_status_aktif"] ?? "";
        $this->status_aktif->AdvancedSearch->SearchOperator2 = $filter["w_status_aktif"] ?? "";
        $this->status_aktif->AdvancedSearch->save();
    ...
    public function basicSearchWhere(bool $default = false): string
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->nama_mitra;
        $searchFlds[] = &$this->alamat;
        $searchFlds[] = &$this->email;
        $searchFlds[] = &$this->telepon;
    ...
    protected function setupSortOrder(): void
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->nama_mitra); // nama_mitra
            $this->updateSort($this->jenis_mitra); // jenis_mitra
            $this->updateSort($this->alamat); // alamat
            $this->updateSort($this->email); // email
            $this->updateSort($this->telepon); // telepon
            $this->updateSort($this->tanggal_bergabung); // tanggal_bergabung
            $this->updateSort($this->status_aktif); // status_aktif
    ...
    protected function resetCmd(): void
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->nama_mitra->setSort("");
                $this->jenis_mitra->setSort("");
                $this->alamat->setSort("");
                $this->email->setSort("");
                $this->telepon->setSort("");
                $this->tanggal_bergabung->setSort("");
                $this->status_aktif->setSort("");
            }
    ...
    public function renderListOptions(): void
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($this->security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"mitra\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $this->language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($this->security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"mitra\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-ask=\"1\" data-btn=\"SaveBtn\">" . $this->language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($this->security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"mitra\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("CopyLink") . "</a>";
                } else {
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fmitralist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fmitralist\" data-key=\"" . $this->keyToJson(true) .
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->mitra_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
			$item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"mitra\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "nama_mitra");
            $this->createColumnOption($option, "jenis_mitra");
            $this->createColumnOption($option, "alamat");
            $this->createColumnOption($option, "email");
            $this->createColumnOption($option, "telepon");
            $this->createColumnOption($option, "tanggal_bergabung");
            $this->createColumnOption($option, "status_aktif");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fmitrasrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fmitrasrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fmitralist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    public function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_mitra", "data-rowtype" => RowType::ADD]);
...
// Load basic search values
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->nama_mitra->TooltipValue = "";

            // jenis_mitra
            $this->jenis_mitra->HrefValue = "";
            $this->jenis_mitra->TooltipValue = "";
            ...
            $this->tanggal_bergabung->TooltipValue = "";

            // status_aktif
            $this->status_aktif->HrefValue = "";
            $this->status_aktif->TooltipValue = "";
        }
	...
	// Begin of add Search Panel Status by Masino Sinaga, October 13, 2024
		...
		if (ReadCookie('mitra_searchpanel') == 'notactive' || ReadCookie('mitra_searchpanel') == "") {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fmitrasrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		} elseif (ReadCookie('mitra_searchpanel') == 'active') {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle active\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fmitrasrch\" aria-pressed=\"true\">" . $this->language->phrase("SearchLink") . "</a>";
		} else {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fmitrasrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		}
        $item->Visible = true;
