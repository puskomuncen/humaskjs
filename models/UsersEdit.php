...
class UsersEdit extends Users
{
    ...
    // CSS class/style
    public string $CurrentPageName = "usersedit";
...
// Get upload files
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->_Password->EditValue = $this->language->phrase("PasswordMask"); // Show as masked password
            ...
            if (!$this->security->canAccess() && $this->security->isLoggedIn()) { // No access permission
                if (SameString($this->_UserID->CurrentValue, CurrentUserID())) {
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        if (!IsMaskedPassword($this->_Password->CurrentValue)) {
            $this->_Password->setDbValueDef($newRow, $this->_Password->CurrentValue, $this->_Password->ReadOnly || Config("ENCRYPTED_PASSWORD") && $oldRow['Password'] == $this->_Password->CurrentValue);
        }
