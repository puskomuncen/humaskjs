...
/**
 * Table class for event
 */
class Event extends DbTable implements LookupTableInterface
{
    ...
    // Fields
    public DbField $event_id;
    public DbField $nama_event;
    public DbField $deskripsi;
    public DbField $tanggal_mulai;
    public DbField $tanggal_selesai;
    public DbField $lokasi;
    public DbField $mitra_id;
    public DbField $peserta_terdaftar;
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "event"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->event_id->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->event_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['event_id'] = &$this->event_id;

        // nama_event
        $this->nama_event = new DbField(
            $this, // Table
            'x_nama_event', // Variable name
            'nama_event', // Name
            '`nama_event`', // Expression
            '`nama_event`', // Basic search expression
            200, // Type
            100, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`nama_event`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->nama_event->InputTextType = "text";
        $this->nama_event->Nullable = false; // NOT NULL field
        $this->nama_event->Required = true; // Required field
        $this->nama_event->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        $this->Fields['nama_event'] = &$this->nama_event;
        ...
        // tanggal_selesai
        $this->tanggal_selesai = new DbField(
            $this, // Table
            'x_tanggal_selesai', // Variable name
            'tanggal_selesai', // Name
            '`tanggal_selesai`', // Expression
            CastDateFieldForLike("`tanggal_selesai`", 0, "DB"), // Basic search expression
            135, // Type
            19, // Size
            0, // Date/Time format
            false, // Is upload field
            '`tanggal_selesai`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->tanggal_selesai->InputTextType = "text";
        $this->tanggal_selesai->Raw = true;
        $this->tanggal_selesai->Nullable = false; // NOT NULL field
        $this->tanggal_selesai->Required = true; // Required field
        $this->tanggal_selesai->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $this->language->phrase("IncorrectDate"));
        $this->tanggal_selesai->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        ...
        $this->lokasi->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        ...
        $this->mitra_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['mitra_id'] = &$this->mitra_id;

        // peserta_terdaftar
        $this->peserta_terdaftar = new DbField(
            $this, // Table
            'x_peserta_terdaftar', // Variable name
            'peserta_terdaftar', // Name
            '`peserta_terdaftar`', // Expression
            '`peserta_terdaftar`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`peserta_terdaftar`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->peserta_terdaftar->addMethod("getDefault", fn() => 0);
        $this->peserta_terdaftar->InputTextType = "text";
        $this->peserta_terdaftar->Raw = true;
        $this->peserta_terdaftar->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->peserta_terdaftar->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['peserta_terdaftar'] = &$this->peserta_terdaftar;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "event";
    }
    ...
    public function insert(array &$row): int|bool
    {
        ...
        if ($result) {
            ...
            $row['event_id'] = $this->event_id->DbValue;
        }
        return $result;
    }
    ...
    public function update(array $row, string|array $where = "", ?array $old = null, bool $currentFilter = true): int|bool
    {
        ...
        if ($success) {
            if (!isset($row['event_id']) && !IsEmpty($this->event_id->CurrentValue)) {
    ...
    public function deleteSql(?array $row, string|array $where = ""): QueryBuilder
    {
        ...
        $where = is_array($where) ? $this->arrayToFilter($where) : $where;
        if ($row) {
            if (array_key_exists('event_id', $row)) {
                AddFilter($where, QuotedName('event_id', $this->Dbid) . '=' . QuotedValue($row['event_id'], $this->event_id->DataType, $this->Dbid));
            }
        }
    ...
    protected function loadDbValues(?array $row)
    {
        ...
        $this->nama_event->DbValue = $row['nama_event'];
        ...
        $this->peserta_terdaftar->DbValue = $row['peserta_terdaftar'];
    }
    ...
    public function getRecordFilter(?array $row = null, bool $current = false): string
    {
        ...
        if (is_array($row)) {
            ...
            $val = !IsEmpty($this->event_id->OldValue) && !$current ? $this->event_id->OldValue : $this->event_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@event_id@", AdjustSql($val), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("eventlist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "eventview" => $this->language->phrase("View"),
            "eventedit" => $this->language->phrase("Edit"),
            "eventadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "EventView",
            Config("API_ADD_ACTION") => "EventAdd",
            Config("API_EDIT_ACTION") => "EventEdit",
            Config("API_DELETE_ACTION") => "EventDelete",
            Config("API_LIST_ACTION") => "EventList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("eventview", $parm);
        } else {
            $url = $this->keyUrl("eventview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("eventedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("eventlist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("eventlist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("eventdelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"event_id\":" . VarToJson($this->event_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
