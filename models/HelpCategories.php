...
/**
 * Table class for help_categories
 */
class HelpCategories extends DbTable implements LookupTableInterface
{
    ...
    public DbField $Category_Description;
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "help_categories"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->Category_Description = new DbField(
            $this, // Table
            'x_Category_Description', // Variable name
            'Category_Description', // Name
            '`Category_Description`', // Expression
            '`Category_Description`', // Basic search expression
            200, // Type
            100, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`Category_Description`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
    ...
    public function getCurrentDetailTable(): ?string
    {
        return Session(AddTabId(PROJECT_NAME . "_" . $this->TableVar . "_" . Config("TABLE_DETAIL_TABLE"))) ?? "";
    }

    public function setCurrentDetailTable(?string $v): void
    {
        Session(AddTabId(PROJECT_NAME . "_" . $this->TableVar . "_" . Config("TABLE_DETAIL_TABLE")), $v);
    }
    ...
    public function getDetailUrl(): string
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "help") {
            $detailUrl = Container("help")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_Category_ID", $this->Category_ID->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "helpcategorieslist";
        }
        return $detailUrl;
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "help_categories";
    }
    ...
    public function insert(array &$row): int|bool
    {
        ...
        if ($result) {
            ...
            $row['Category_ID'] = $this->Category_ID->DbValue;
        }
        return $result;
    }
    ...
    public function update(array $row, string|array $where = "", ?array $old = null, bool $currentFilter = true): int|bool
    {
        ...
        if ($success) {
            if (!isset($row['Category_ID']) && !IsEmpty($this->Category_ID->CurrentValue)) {
    ...
    public function deleteSql(?array $row, string|array $where = ""): QueryBuilder
    {
        ...
        if ($row) {
            if (array_key_exists('Category_ID', $row)) {
                AddFilter($where, QuotedName('Category_ID', $this->Dbid) . '=' . QuotedValue($row['Category_ID'], $this->Category_ID->DataType, $this->Dbid));
            }
        }
    ...
    public function getRecordFilter(?array $row = null, bool $current = false): string
    {
        ...
        if (is_array($row)) {
            ...
            $val = !IsEmpty($this->Category_ID->OldValue) && !$current ? $this->Category_ID->OldValue : $this->Category_ID->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@Category_ID@", AdjustSql($val), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("helpcategorieslist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "helpcategoriesview" => $this->language->phrase("View"),
            "helpcategoriesedit" => $this->language->phrase("Edit"),
            "helpcategoriesadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "HelpCategoriesView",
            Config("API_ADD_ACTION") => "HelpCategoriesAdd",
            Config("API_EDIT_ACTION") => "HelpCategoriesEdit",
            Config("API_DELETE_ACTION") => "HelpCategoriesDelete",
            Config("API_LIST_ACTION") => "HelpCategoriesList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("helpcategoriesview", $parm);
        } else {
            $url = $this->keyUrl("helpcategoriesview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "helpcategoriesadd?" . $parm;
        } else {
            $url = "helpcategoriesadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("helpcategoriesedit", $parm);
        } else {
            $url = $this->keyUrl("helpcategoriesedit", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("helpcategorieslist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("helpcategoriesadd", $parm);
        } else {
            $url = $this->keyUrl("helpcategoriesadd", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("helpcategorieslist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("helpcategoriesdelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"Category_ID\":" . VarToJson($this->Category_ID->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->Category_Description);
                } else {
                    $doc->exportCaption($this->Category_ID);
                    $doc->exportCaption($this->_Language);
                    $doc->exportCaption($this->Category_Description);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->Category_Description);
                    } else {
                        $doc->exportField($this->Category_ID);
                        $doc->exportField($this->_Language);
                        $doc->exportField($this->Category_Description);
    ...
    public function renderLookupForView(string $name, mixed $value): mixed
    {
        ...
        if ($name == "Category_Description") {
            $clone = $this->Category_Description->getClone()->setViewValue($value);
