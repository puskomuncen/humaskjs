...
class HelpList extends Help
{
    ...
    // Page object name
    public string $PageObjName = "HelpList";
    ...
    public string $FormName = "fhelplist";

    // CSS class/style
    public string $CurrentPageName = "helplist";
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->MultiDeleteUrl = "helpdelete";
        $this->MultiUpdateUrl = "helpupdate";
...
// Is modal
		...
		if (ReadCookie('help_searchpanel') == 'notactive' || ReadCookie('help_searchpanel') == "") {
			RemoveClass($this->SearchPanelClass, "show");
			AppendClass($this->SearchPanelClass, "collapse");
		} elseif (ReadCookie('help_searchpanel') == 'active') {
    ...
    public function getFilterList(): string
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fhelpsrch");
        }
        $filterList = Concat($filterList, $this->Help_ID->AdvancedSearch->toJson(), ","); // Field Help_ID
        ...
        $filterList = Concat($filterList, $this->Display_in_Page->AdvancedSearch->toJson(), ","); // Field Display_in_Page
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fhelpsrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        // Field Help_ID
        $this->Help_ID->AdvancedSearch->SearchValue = $filter["x_Help_ID"] ?? "";
        $this->Help_ID->AdvancedSearch->SearchOperator = $filter["z_Help_ID"] ?? "";
        $this->Help_ID->AdvancedSearch->SearchCondition = $filter["v_Help_ID"] ?? "";
        $this->Help_ID->AdvancedSearch->SearchValue2 = $filter["y_Help_ID"] ?? "";
        $this->Help_ID->AdvancedSearch->SearchOperator2 = $filter["w_Help_ID"] ?? "";
        $this->Help_ID->AdvancedSearch->save();
        ...
        // Field Display_in_Page
        $this->Display_in_Page->AdvancedSearch->SearchValue = $filter["x_Display_in_Page"] ?? "";
        $this->Display_in_Page->AdvancedSearch->SearchOperator = $filter["z_Display_in_Page"] ?? "";
        $this->Display_in_Page->AdvancedSearch->SearchCondition = $filter["v_Display_in_Page"] ?? "";
        $this->Display_in_Page->AdvancedSearch->SearchValue2 = $filter["y_Display_in_Page"] ?? "";
        $this->Display_in_Page->AdvancedSearch->SearchOperator2 = $filter["w_Display_in_Page"] ?? "";
        $this->Display_in_Page->AdvancedSearch->save();
    ...
    public function basicSearchWhere(bool $default = false): string
    {
        ...
        $searchFlds[] = &$this->Display_in_Page;
        $searchFlds[] = &$this->Updated_By;
    ...
    protected function resetCmd(): void
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->Help_ID->setSort("");
                $this->_Language->setSort("");
                $this->Topic->setSort("");
                $this->Description->setSort("");
                $this->Category->setSort("");
                $this->Order->setSort("");
                $this->Display_in_Page->setSort("");
    ...
    public function renderListOptions(): void
    {
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fhelplist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fhelplist\" data-key=\"" . $this->keyToJson(true) .
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->Help_ID->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions(): void
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fhelpsrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fhelpsrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fhelplist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
...
// Load basic search values
	...
	// Begin of add Search Panel Status by Masino Sinaga, October 13, 2024
		...
		if (ReadCookie('help_searchpanel') == 'notactive' || ReadCookie('help_searchpanel') == "") {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fhelpsrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		} elseif (ReadCookie('help_searchpanel') == 'active') {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle active\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fhelpsrch\" aria-pressed=\"true\">" . $this->language->phrase("SearchLink") . "</a>";
		} else {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fhelpsrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		}
        $item->Visible = true;
