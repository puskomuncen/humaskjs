 ...
 * Page class
 */
class KontakMitraList extends KontakMitra
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "KontakMitraList";
    ...
    public string $FormName = "fkontak_mitralist";

    // CSS class/style
    public string $CurrentPageName = "kontakmitralist";
    ...
    {
        $this->kontak_id->setVisibility();
        $this->mitra_id->setVisibility();
        $this->nama_kontak->setVisibility();
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->TableVar = 'kontak_mitra';
        $this->TableName = 'kontak_mitra';
        ...
        // Table object (kontak_mitra)
        if (!isset($GLOBALS["kontak_mitra"]) || $GLOBALS["kontak_mitra"]::class == PROJECT_NAMESPACE . "kontak_mitra") {
            $GLOBALS["kontak_mitra"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "kontakmitraadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "kontakmitradelete";
        $this->MultiUpdateUrl = "kontakmitraupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'kontak_mitra');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "kontakmitraview"); // If View page, no primary button
                } else { // List page
...
// Is modal
        ...
        if ($this->IsModal) {
            $this->FormName = "fkontak_mitragrid";
        }
		...
		if (ReadCookie('kontak_mitra_searchpanel') == 'notactive' || ReadCookie('kontak_mitra_searchpanel') == "") {
			RemoveClass($this->SearchPanelClass, "show");
			AppendClass($this->SearchPanelClass, "collapse");
		} elseif (ReadCookie('kontak_mitra_searchpanel') == 'active') {
    ...
    public function getFilterList(): string
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fkontak_mitrasrch");
        }
        $filterList = Concat($filterList, $this->kontak_id->AdvancedSearch->toJson(), ","); // Field kontak_id
        $filterList = Concat($filterList, $this->mitra_id->AdvancedSearch->toJson(), ","); // Field mitra_id
        $filterList = Concat($filterList, $this->nama_kontak->AdvancedSearch->toJson(), ","); // Field nama_kontak
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fkontak_mitrasrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        // Field kontak_id
        $this->kontak_id->AdvancedSearch->SearchValue = $filter["x_kontak_id"] ?? "";
        $this->kontak_id->AdvancedSearch->SearchOperator = $filter["z_kontak_id"] ?? "";
        $this->kontak_id->AdvancedSearch->SearchCondition = $filter["v_kontak_id"] ?? "";
        $this->kontak_id->AdvancedSearch->SearchValue2 = $filter["y_kontak_id"] ?? "";
        $this->kontak_id->AdvancedSearch->SearchOperator2 = $filter["w_kontak_id"] ?? "";
        $this->kontak_id->AdvancedSearch->save();
        ...
        // Field nama_kontak
        $this->nama_kontak->AdvancedSearch->SearchValue = $filter["x_nama_kontak"] ?? "";
        $this->nama_kontak->AdvancedSearch->SearchOperator = $filter["z_nama_kontak"] ?? "";
        $this->nama_kontak->AdvancedSearch->SearchCondition = $filter["v_nama_kontak"] ?? "";
        $this->nama_kontak->AdvancedSearch->SearchValue2 = $filter["y_nama_kontak"] ?? "";
        $this->nama_kontak->AdvancedSearch->SearchOperator2 = $filter["w_nama_kontak"] ?? "";
        $this->nama_kontak->AdvancedSearch->save();
    ...
    protected function setupSortOrder(): void
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->kontak_id); // kontak_id
            $this->updateSort($this->mitra_id); // mitra_id
            $this->updateSort($this->nama_kontak); // nama_kontak
    ...
    protected function resetCmd(): void
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->kontak_id->setSort("");
                $this->mitra_id->setSort("");
                $this->nama_kontak->setSort("");
    ...
    public function renderListOptions(): void
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($this->security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"kontak_mitra\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $this->language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($this->security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"kontak_mitra\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-ask=\"1\" data-btn=\"SaveBtn\">" . $this->language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($this->security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"kontak_mitra\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("CopyLink") . "</a>";
                } else {
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fkontak_mitralist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fkontak_mitralist\" data-key=\"" . $this->keyToJson(true) .
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->kontak_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
			$item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"kontak_mitra\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "kontak_id");
            $this->createColumnOption($option, "mitra_id");
            $this->createColumnOption($option, "nama_kontak");
            $this->createColumnOption($option, "jabatan");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fkontak_mitrasrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fkontak_mitrasrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fkontak_mitralist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    public function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_kontak_mitra", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_kontak_mitra",
...
// Load basic search values
    ...
    public function loadRowValues(array|bool|null $row = null): void
    {
        ...
        $this->kontak_id->setDbValue($row['kontak_id']);
        $this->mitra_id->setDbValue($row['mitra_id']);
        $this->nama_kontak->setDbValue($row['nama_kontak']);
    ...
    {
        $row = [];
        $row['kontak_id'] = $this->kontak_id->DefaultValue;
        $row['mitra_id'] = $this->mitra_id->DefaultValue;
        $row['nama_kontak'] = $this->nama_kontak->DefaultValue;
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            // kontak_id
            $this->kontak_id->ViewValue = $this->kontak_id->CurrentValue;
            ...
            // nama_kontak
            $this->nama_kontak->ViewValue = $this->nama_kontak->CurrentValue;
            ...
            // kontak_id
            $this->kontak_id->HrefValue = "";
            $this->kontak_id->TooltipValue = "";
            ...
            // nama_kontak
            $this->nama_kontak->HrefValue = "";
            $this->nama_kontak->TooltipValue = "";
	...
	// Begin of add Search Panel Status by Masino Sinaga, October 13, 2024
		...
		if (ReadCookie('kontak_mitra_searchpanel') == 'notactive' || ReadCookie('kontak_mitra_searchpanel') == "") {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fkontak_mitrasrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		} elseif (ReadCookie('kontak_mitra_searchpanel') == 'active') {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle active\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fkontak_mitrasrch\" aria-pressed=\"true\">" . $this->language->phrase("SearchLink") . "</a>";
		} else {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fkontak_mitrasrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		}
        $item->Visible = true;
