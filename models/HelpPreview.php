 ...
 * Page class
 */
class HelpPreview extends Help
{
    use MessagesTrait;
    ...
    // Page object name
    public string $PageObjName = "HelpPreview";
    ...
    // CSS class/style
    public string $CurrentPageName = "helppreview";
...
// Use layout
    ...
    protected function masterKeyUrl(): string
    {
        ...
        if ($masterTblVar == "help_categories") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=help_categories&" . GetForeignKeyUrl("fk_Category_ID", $this->Category->QueryStringValue) . "";
        }
        return $url;
    }
    ...
    protected function setupForeignKeys(array $keys): void
    {
        $masterTblVar = Get("t", "");
        if ($masterTblVar == "help_categories") {
            $this->Category->setQueryStringValue($keys[0] ?? "");
        }
    }
