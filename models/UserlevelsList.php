 ...
 * Page class
 */
class UserlevelsList extends Userlevels
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "UserlevelsList";
    ...
    public string $FormName = "fuserlevelslist";

    // CSS class/style
    public string $CurrentPageName = "userlevelslist";
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        // Initialize URLs
        $this->AddUrl = "userlevelsadd?" . Config("TABLE_SHOW_DETAIL") . "=";
...
// Is modal
		...
		if (ReadCookie('userlevels_searchpanel') == 'notactive' || ReadCookie('userlevels_searchpanel') == "") {
			RemoveClass($this->SearchPanelClass, "show");
			AppendClass($this->SearchPanelClass, "collapse");
		} elseif (ReadCookie('userlevels_searchpanel') == 'active') {
    ...
    public function getFilterList(): string
    {
        ...
        $filterList = Concat($filterList, $this->Hierarchy->AdvancedSearch->toJson(), ","); // Field Hierarchy
        $filterList = Concat($filterList, $this->Level_Origin->AdvancedSearch->toJson(), ","); // Field Level_Origin
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fuserlevelssrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        // Field Hierarchy
        $this->Hierarchy->AdvancedSearch->SearchValue = $filter["x_Hierarchy"] ?? "";
        $this->Hierarchy->AdvancedSearch->SearchOperator = $filter["z_Hierarchy"] ?? "";
        $this->Hierarchy->AdvancedSearch->SearchCondition = $filter["v_Hierarchy"] ?? "";
        $this->Hierarchy->AdvancedSearch->SearchValue2 = $filter["y_Hierarchy"] ?? "";
        $this->Hierarchy->AdvancedSearch->SearchOperator2 = $filter["w_Hierarchy"] ?? "";
        $this->Hierarchy->AdvancedSearch->save();

        // Field Level_Origin
        $this->Level_Origin->AdvancedSearch->SearchValue = $filter["x_Level_Origin"] ?? "";
        $this->Level_Origin->AdvancedSearch->SearchOperator = $filter["z_Level_Origin"] ?? "";
        $this->Level_Origin->AdvancedSearch->SearchCondition = $filter["v_Level_Origin"] ?? "";
        $this->Level_Origin->AdvancedSearch->SearchValue2 = $filter["y_Level_Origin"] ?? "";
        $this->Level_Origin->AdvancedSearch->SearchOperator2 = $filter["w_Level_Origin"] ?? "";
        $this->Level_Origin->AdvancedSearch->save();
    ...
    protected function setupSortOrder(): void
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->Hierarchy); // Hierarchy
            $this->updateSort($this->Level_Origin); // Level_Origin
    ...
    protected function resetCmd(): void
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->ID->setSort("");
                $this->Name->setSort("");
                $this->Hierarchy->setSort("");
                $this->Level_Origin->setSort("");
            }
    ...
    protected function setupListOptions(): void
    {
        ...
        $item->CssClass = "text-nowrap";
        $item->Visible = $this->security->allowList(CurrentProjectID() . 'users');
        $item->OnLeft = true;
        ...
        $item->CssClass = "text-nowrap";
        $item->Visible = $this->security->canGrant();
        $item->OnLeft = true;
    ...
    public function renderListOptions(): void
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($this->security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"userlevels\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $this->language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($this->security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"userlevels\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-ask=\"1\" data-btn=\"SaveBtn\">" . $this->language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($this->security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"userlevels\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("CopyLink") . "</a>";
                } else {
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fuserlevelslist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fuserlevelslist\" data-key=\"" . $this->keyToJson(true) .
        ...
        if ($this->security->allowList(CurrentProjectID() . 'users')) {
            $body = $this->language->phrase("DetailLink") . $this->language->tablePhrase("users", "TblCaption");
            if (!$this->ShowMultipleDetails) { // Skip loading record count if show multiple details
                $detailTbl = Container("users");
            ...
            if ($detailPage->DetailView && $this->security->canView() && $this->security->allowView(CurrentProjectID() . 'userlevels')) {
            ...
            if ($detailPage->DetailEdit && $this->security->canEdit() && $this->security->allowEdit(CurrentProjectID() . 'userlevels')) {
            ...
            if ($detailPage->DetailAdd && $this->security->canAdd() && $this->security->allowAdd(CurrentProjectID() . 'userlevels')) {
        ...
        if ($this->security->hasUserLevelID($this->ID->CurrentValue) || $this->ID->CurrentValue < AdvancedSecurity::DEFAULT_USER_LEVEL_ID && $this->ID->CurrentValue != AdvancedSecurity::ANONYMOUS_USER_LEVEL_ID) {
            $opt->Body = "-";
        } else {
            $opt->Body = "<a class=\"ew-row-link ew-user-permission\" title=\"" . HtmlTitle($this->language->phrase("Permission")) . "\" data-caption=\"" . HtmlTitle($this->language->phrase("Permission")) . "\" href=\"" . HtmlEncode("userpriv?ID=" . $this->ID->CurrentValue) . "\">" . $this->language->phrase("Permission") . "</a>";
        }
        ...
        // Column "detail_users"
        if ($this->DetailPages?->getItem("users")?->Visible && $this->security->allowList(CurrentProjectID() . 'users')) {
            $link = "";
            ...
            $url = "userspreview?t=userlevels&f=" . Encrypt($detailFilter . "|" . implode("|", array_values($masterKeys)));
            $btngrp = "<div data-table=\"users\" data-url=\"" . $url . "\" class=\"ew-detail-btn-group btn-group btn-group-sm d-none\">";
            if ($this->security->allowList(CurrentProjectID() . 'userlevels')) {
                $label = $this->language->tablePhrase("users", "TblCaption");
                ...
                $title = $this->language->tablePhrase("users", "TblCaption");
            ...
            if ($detailPageObj->DetailView && $this->security->canView() && $this->security->allowView(CurrentProjectID() . 'userlevels')) {
                $caption = $this->language->phrase("MasterDetailViewLink");
                $viewurl = GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=users"));
            ...
            if ($detailPageObj->DetailEdit && $this->security->canEdit() && $this->security->allowEdit(CurrentProjectID() . 'userlevels')) {
                $caption = $this->language->phrase("MasterDetailEditLink");
                $editurl = GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=users"));
            ...
            if ($detailPageObj->DetailAdd && $this->security->canAdd() && $this->security->allowAdd(CurrentProjectID() . 'userlevels')) {
                $caption = $this->language->phrase("MasterDetailCopyLink");
                $copyurl = GetUrl($this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=users"));
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
			$item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"userlevels\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("AddLink") . "</a>";
        } else {
        ...
        $item->Visible = ($detailPage->DetailAdd && $this->security->allowAdd(CurrentProjectID() . 'userlevels') && $this->security->canAdd());
        if ($item->Visible) {
            if ($detailTableLink != "") {
                $detailTableLink .= ",";
            }
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "Hierarchy");
            $this->createColumnOption($option, "Level_Origin");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fuserlevelssrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fuserlevelssrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fuserlevelslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    public function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_userlevels", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_userlevels",
...
// Load basic search values
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->Hierarchy->TooltipValue = "";

            // Level_Origin
            $this->Level_Origin->HrefValue = "";
            $this->Level_Origin->TooltipValue = "";
        }
	...
	// Begin of add Search Panel Status by Masino Sinaga, October 13, 2024
		...
		if (ReadCookie('userlevels_searchpanel') == 'notactive' || ReadCookie('userlevels_searchpanel') == "") {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fuserlevelssrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		} elseif (ReadCookie('userlevels_searchpanel') == 'active') {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle active\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fuserlevelssrch\" aria-pressed=\"true\">" . $this->language->phrase("SearchLink") . "</a>";
		} else {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fuserlevelssrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		}
        $item->Visible = true;
    ...
    {
        $this->CustomActions["copy_permissions"] = new ListAction("copy_permissions", "Copy Permissions", Allowed: IsLoggedIn() && IsAdmin(), Select: ActionType::CUSTOM);
    }
    ...
    {
        $header = "Make sure you have already defined <strong>Origin Level</strong> for your desired <strong>User Level</strong>, by clicking on <strong>Pencil</strong> icon, and then save the changes. After that, click on <strong>Copy Permissions</strong> icon, next to <strong>User</strong> icon to copy permissions from Origin Level to the current target User Level ...";
    }
    ...
    {
        $item = &$this->ListOptions->add("copy_permissions");
        $item->Header = "Copy Permissions";
        $item->OnLeft = true;
        $item->moveTo(7);
    }
    ...
    {
        if (!empty($this->Level_Origin->CurrentValue) || $this->Level_Origin->CurrentValue == "0") {
    		$this->ListOptions->Items["copy_permissions"]->Body = "<a class=\"btn btn-default ew-row-link ew-action ew-list-action\" href=\"javascript:void(0);\" onclick=\"return ew.submitAction(event, {action: 'copy_permissions', method: 'A', msg: 'Copy Permissions from level <strong>" . GetUserLevelName($this->Level_Origin->CurrentValue) . "</strong> to level <strong>" . $this->Name->ViewValue . "</strong>?', key: " . $this->keyToJson(true) . "});\" data-caption=\"Copy Permissions\" title=\"Copy Permissions\">" . Language()->phrase("CopyPermissions") . "</a>";
        }
        if ($this->ID->CurrentValue == "-2" || $this->ID->CurrentValue == "-1" || $this->ID->CurrentValue == "0") {
    ...
    public function rowCustomAction(string $action, array $row): bool
    {
        ...
        if ($action == "copy_permissions") {
            if (!empty($row["Level_Origin"]) || $row["Level_Origin"] == "0" ) {
                // first, delete all the permissions
                ExecuteStatement("DELETE FROM " . Config("USER_LEVEL_PRIV_TABLE") . " WHERE " . Config("USER_LEVEL_PRIV_USER_LEVEL_ID_FIELD") . " = " . $row["ID"]);
                // then, insert the permissions based on the origin user level
                $result = ExecuteStatement("INSERT INTO " . Config("USER_LEVEL_PRIV_TABLE") . " (" . Config("USER_LEVEL_PRIV_USER_LEVEL_ID_FIELD") . ", " . Config("USER_LEVEL_PRIV_TABLE_NAME_FIELD") . ", " . Config("USER_LEVEL_PRIV_PRIV_FIELD") . ") SELECT " . $row["ID"] . ", " . Config("USER_LEVEL_PRIV_TABLE_NAME_FIELD") . ", " . Config("USER_LEVEL_PRIV_PRIV_FIELD") . " FROM " . Config("USER_LEVEL_PRIV_TABLE") . " WHERE " . Config("USER_LEVEL_PRIV_USER_LEVEL_ID_FIELD") . " = " . $row["Level_Origin"]);
                if (!$result) { // Failure
                    $this->setFailureMessage("Failed to copy permissions for user level <strong>" . $row["Name"] . "</strong>.");
                    return false; // Abort and rollback
                } elseif ($this->SelectedIndex == $this->SelectedCount) { // Last row
                    $this->setSuccessMessage("Permissions for User Level <strong>" . $row["Name"] . "</strong> has successfully been defined.");                
                } 
                //return true; // Success
            } else {
                $this->setFailureMessage("User Level <strong>" . $row["Name"] . "</strong> still empty. <br><br>Make sure the Origin Level has been selected by changing the field.");
                return false; // Abort and rollback
            }
        }
        return true;
