...
class HelpEdit extends Help
{
    ...
    // Page object name
    public string $PageObjName = "HelpEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "helpedit";
...
// Get upload files
    ...
    protected function loadFormValues(): void
    {
        ...
        // Check field name 'Help_ID' before field var 'x_Help_ID'
        $val = $this->getFormValue("Help_ID", null) ?? $this->getFormValue("x_Help_ID", null);
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            if ($this->Help_ID->Visible && $this->Help_ID->Required) {
                if (!$this->Help_ID->IsDetailKey && IsEmpty($this->Help_ID->FormValue)) {
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // Order
        $this->Order->setDbValueDef($newRow, $this->Order->CurrentValue, $this->Order->ReadOnly);

        // Display_in_Page
        $this->Display_in_Page->setDbValueDef($newRow, $this->Display_in_Page->CurrentValue, $this->Display_in_Page->ReadOnly);

        // Updated_By
        $this->Updated_By->setDbValueDef($newRow, $this->Updated_By->CurrentValue, $this->Updated_By->ReadOnly);

        // Last_Updated
        $this->Last_Updated->setDbValueDef($newRow, UnFormatDateTime($this->Last_Updated->CurrentValue, $this->Last_Updated->formatPattern()), $this->Last_Updated->ReadOnly);
        return $newRow;
    }
    ...
    protected function restoreEditFormFromRow(array $row): void
    {
        ...
        if (isset($row['Order'])) { // Order
            $this->Order->CurrentValue = $row['Order'];
        }
        if (isset($row['Display_in_Page'])) { // Display_in_Page
            $this->Display_in_Page->CurrentValue = $row['Display_in_Page'];
        }
        if (isset($row['Updated_By'])) { // Updated_By
            $this->Updated_By->CurrentValue = $row['Updated_By'];
        }
        if (isset($row['Last_Updated'])) { // Last_Updated
            $this->Last_Updated->CurrentValue = $row['Last_Updated'];
        }
    }
