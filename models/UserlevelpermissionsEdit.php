 ...
 * Page class
 */
class UserlevelpermissionsEdit extends Userlevelpermissions
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "UserlevelpermissionsEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "userlevelpermissionsedit";
...
// Is modal
        ...
        if (IsApi()) {
            ...
            if (($keyValue = Get("_TableName") ?? Key(1) ?? Route(3)) !== null) {
...
// Get upload files
    ...
    protected function loadFormValues(): void
    {
        ...
        $val = $this->getFormValue("UserLevelID", null) ?? $this->getFormValue("x_UserLevelID", null);
        if (!$this->UserLevelID->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->UserLevelID->Visible = false; // Disable update for API request
            } else {
                $this->UserLevelID->setFormValue($val, true, $validate);
            }
        }
        if ($this->hasFormValue("o_UserLevelID")) {
            $this->UserLevelID->setOldValue($this->getFormValue("o_UserLevelID"));
        }

        // Check field name 'TableName' before field var 'x__TableName'
        $val = $this->getFormValue("TableName", null) ?? $this->getFormValue("x__TableName", null);
        ...
        if ($this->hasFormValue("o__TableName")) {
            $this->_TableName->setOldValue($this->getFormValue("o__TableName"));
        }

        // Check field name 'Permission' before field var 'x__Permission'
        $val = $this->getFormValue("Permission", null) ?? $this->getFormValue("x__Permission", null);
        if (!$this->_Permission->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->_Permission->Visible = false; // Disable update for API request
            } else {
                $this->_Permission->setFormValue($val, true, $validate);
            }
        }
    }
    ...
    public function restoreFormValues(): void
    {
        ...
        $this->_Permission->CurrentValue = $this->_Permission->FormValue;
    }
    ...
    public function renderRow(): void
    {
        ...
        // TableName
        $this->_TableName->RowCssClass = "row";

        // Permission
        $this->_Permission->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // TableName
            $this->_TableName->setupEditAttributes();
            $this->_TableName->EditValue = !$this->_TableName->Raw ? HtmlDecode($this->_TableName->CurrentValue) : $this->_TableName->CurrentValue;
            $this->_TableName->PlaceHolder = RemoveHtml($this->_TableName->caption());

            // Permission
            $this->_Permission->setupEditAttributes();
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            if ($this->UserLevelID->Visible && $this->UserLevelID->Required) {
                if (!$this->UserLevelID->IsDetailKey && IsEmpty($this->UserLevelID->FormValue)) {
            ...
            if ($this->_TableName->Visible && $this->_TableName->Required) {
                if (!$this->_TableName->IsDetailKey && IsEmpty($this->_TableName->FormValue)) {
                    $this->_TableName->addErrorMessage(str_replace("%s", $this->_TableName->caption(), $this->_TableName->RequiredErrorMessage));
                }
            }
            if ($this->_Permission->Visible && $this->_Permission->Required) {
                if (!$this->_Permission->IsDetailKey && IsEmpty($this->_Permission->FormValue)) {
                    $this->_Permission->addErrorMessage(str_replace("%s", $this->_Permission->caption(), $this->_Permission->RequiredErrorMessage));
                }
            }
            if (!CheckInteger($this->_Permission->FormValue)) {
                $this->_Permission->addErrorMessage($this->_Permission->getErrorMessage(false));
            }
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // UserLevelID
        $this->UserLevelID->setDbValueDef($newRow, $this->UserLevelID->CurrentValue, $this->UserLevelID->ReadOnly);

        // TableName
        $this->_TableName->setDbValueDef($newRow, $this->_TableName->CurrentValue, $this->_TableName->ReadOnly);

        // Permission
        $this->_Permission->setDbValueDef($newRow, $this->_Permission->CurrentValue, $this->_Permission->ReadOnly);
        return $newRow;
    }
    ...
    protected function restoreEditFormFromRow(array $row): void
    {
        ...
        if (isset($row['TableName'])) { // TableName
            $this->_TableName->CurrentValue = $row['TableName'];
        }
        if (isset($row['Permission'])) { // Permission
            $this->_Permission->CurrentValue = $row['Permission'];
        }
    }
