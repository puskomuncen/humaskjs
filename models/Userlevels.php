 ...
 */
class Userlevels extends DbTable implements LookupTableInterface
{
    ...
    public DbField $ID;
    public DbField $Name;
    public DbField $Hierarchy;
    public DbField $Level_Origin;
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "userlevels"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // Hierarchy
        $this->Hierarchy = new DbField(
            $this, // Table
            'x_Hierarchy', // Variable name
            'Hierarchy', // Name
            '`Hierarchy`', // Expression
            '`Hierarchy`', // Basic search expression
            200, // Type
            255, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`Hierarchy`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->Hierarchy->InputTextType = "text";
        $this->Hierarchy->setSelectMultiple(true); // Select multiple
        ...
        $this->Fields['Hierarchy'] = &$this->Hierarchy;

        // Level_Origin
        $this->Level_Origin = new DbField(
            $this, // Table
            'x_Level_Origin', // Variable name
            'Level_Origin', // Name
            '`Level_Origin`', // Expression
            '`Level_Origin`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`Level_Origin`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->Level_Origin->InputTextType = "text";
        $this->Level_Origin->Raw = true;
        $this->Level_Origin->setSelectMultiple(false); // Select one
        $this->Level_Origin->UsePleaseSelect = true; // Use PleaseSelect by default
        $this->Level_Origin->PleaseSelectText = $this->language->phrase("PleaseSelect"); // "PleaseSelect" text
        global $CurrentLanguage;
        switch ($CurrentLanguage) {
            case "en-US":
        ...
        $this->Level_Origin->SearchOperators = ["=", "<>", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['Level_Origin'] = &$this->Level_Origin;
    ...
    public function getDetailUrl(): string
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "users") {
            $detailUrl = Container("users")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
    ...
    protected function loadDbValues(?array $row)
    {
        ...
        $this->Hierarchy->DbValue = $row['Hierarchy'];
        $this->Level_Origin->DbValue = $row['Level_Origin'];
    }
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("userlevelslist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "userlevelsview" => $this->language->phrase("View"),
            "userlevelsedit" => $this->language->phrase("Edit"),
            "userlevelsadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->Hierarchy);
                    $doc->exportCaption($this->Level_Origin);
                } else {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->Hierarchy);
                        $doc->exportField($this->Level_Origin);
                    } else {
    ...
    public function renderLookupForView(string $name, mixed $value): mixed
    {
        ...
        if ($name == "Name") {
            $clone = $this->Name->getClone()->setViewValue($value);
            $clone->ViewValue = $clone->CurrentValue;
            if ($this->security->getUserLevelName($this->ID->CurrentValue) != "") {
                $clone->ViewValue = $this->security->getUserLevelName($this->ID->CurrentValue);
            }
    ...
    public function rowSelected(array &$row): void
    {
        if (CurrentPageID() == "edit") {
            if ($row["ID"] == "-2" || $row["ID"] == "-1" || $row["ID"] == "0") {
                $this->setFailureMessage("User level Anonymous, Administrator, and Default cannot be edited!");
                $this->terminate("userlevelslist");
            }
        }
    }
    ...
    public function rowUpdating(array $oldRow, array &$newRow): ?bool
    {
        ...
        // To cancel, set return value to false
        // To skip for grid insert/update, set return value to null
        if ($newRow["Level_Origin"] == $oldRow["ID"]) {
            $this->setFailureMessage("Origin Level cannot be the same with current User Level, please change Origin Level to another!");
            return false;
        }
        return true;
    }
    ...
    public function lookupSelecting(DbField $field, string &$filter): void
    {
        ...
        // Enter your code here
        if ($field->Name == "Level_Origin") {
            AddFilter($filter, "ID <> -1");
        }
    }
