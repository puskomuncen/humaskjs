...
class EventList extends Event
{
    ...
    // Page object name
    public string $PageObjName = "EventList";
    ...
    public string $FormName = "feventlist";

    // CSS class/style
    public string $CurrentPageName = "eventlist";
    ...
    public function setVisibility(): void
    {
        ...
        $this->peserta_terdaftar->setVisibility();
    }
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        // Table object (event)
        if (!isset($GLOBALS["event"]) || $GLOBALS["event"]::class == PROJECT_NAMESPACE . "event") {
            $GLOBALS["event"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "eventadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "eventdelete";
        $this->MultiUpdateUrl = "eventupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'event');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "eventview"); // If View page, no primary button
                } else { // List page
...
// Is modal
		...
		if (ReadCookie('event_searchpanel') == 'notactive' || ReadCookie('event_searchpanel') == "") {
			RemoveClass($this->SearchPanelClass, "show");
			AppendClass($this->SearchPanelClass, "collapse");
		} elseif (ReadCookie('event_searchpanel') == 'active') {
    ...
    public function getFilterList(): string
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("feventsrch");
        }
        $filterList = Concat($filterList, $this->event_id->AdvancedSearch->toJson(), ","); // Field event_id
        $filterList = Concat($filterList, $this->nama_event->AdvancedSearch->toJson(), ","); // Field nama_event
        ...
        $filterList = Concat($filterList, $this->tanggal_selesai->AdvancedSearch->toJson(), ","); // Field tanggal_selesai
        ...
        $filterList = Concat($filterList, $this->peserta_terdaftar->AdvancedSearch->toJson(), ","); // Field peserta_terdaftar
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("feventsrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        // Field event_id
        $this->event_id->AdvancedSearch->SearchValue = $filter["x_event_id"] ?? "";
        $this->event_id->AdvancedSearch->SearchOperator = $filter["z_event_id"] ?? "";
        $this->event_id->AdvancedSearch->SearchCondition = $filter["v_event_id"] ?? "";
        $this->event_id->AdvancedSearch->SearchValue2 = $filter["y_event_id"] ?? "";
        $this->event_id->AdvancedSearch->SearchOperator2 = $filter["w_event_id"] ?? "";
        $this->event_id->AdvancedSearch->save();

        // Field nama_event
        $this->nama_event->AdvancedSearch->SearchValue = $filter["x_nama_event"] ?? "";
        $this->nama_event->AdvancedSearch->SearchOperator = $filter["z_nama_event"] ?? "";
        $this->nama_event->AdvancedSearch->SearchCondition = $filter["v_nama_event"] ?? "";
        $this->nama_event->AdvancedSearch->SearchValue2 = $filter["y_nama_event"] ?? "";
        $this->nama_event->AdvancedSearch->SearchOperator2 = $filter["w_nama_event"] ?? "";
        $this->nama_event->AdvancedSearch->save();
        ...
        $this->tanggal_selesai->AdvancedSearch->SearchValue = $filter["x_tanggal_selesai"] ?? "";
        $this->tanggal_selesai->AdvancedSearch->SearchOperator = $filter["z_tanggal_selesai"] ?? "";
        $this->tanggal_selesai->AdvancedSearch->SearchCondition = $filter["v_tanggal_selesai"] ?? "";
        $this->tanggal_selesai->AdvancedSearch->SearchValue2 = $filter["y_tanggal_selesai"] ?? "";
        $this->tanggal_selesai->AdvancedSearch->SearchOperator2 = $filter["w_tanggal_selesai"] ?? "";
        $this->tanggal_selesai->AdvancedSearch->save();
        ...
        // Field peserta_terdaftar
        $this->peserta_terdaftar->AdvancedSearch->SearchValue = $filter["x_peserta_terdaftar"] ?? "";
        $this->peserta_terdaftar->AdvancedSearch->SearchOperator = $filter["z_peserta_terdaftar"] ?? "";
        $this->peserta_terdaftar->AdvancedSearch->SearchCondition = $filter["v_peserta_terdaftar"] ?? "";
        $this->peserta_terdaftar->AdvancedSearch->SearchValue2 = $filter["y_peserta_terdaftar"] ?? "";
        $this->peserta_terdaftar->AdvancedSearch->SearchOperator2 = $filter["w_peserta_terdaftar"] ?? "";
        $this->peserta_terdaftar->AdvancedSearch->save();
    ...
    public function basicSearchWhere(bool $default = false): string
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->nama_event;
        $searchFlds[] = &$this->deskripsi;
        $searchFlds[] = &$this->lokasi;
    ...
    protected function setupSortOrder(): void
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->event_id); // event_id
            $this->updateSort($this->nama_event); // nama_event
            $this->updateSort($this->deskripsi); // deskripsi
            $this->updateSort($this->tanggal_mulai); // tanggal_mulai
            $this->updateSort($this->tanggal_selesai); // tanggal_selesai
            $this->updateSort($this->lokasi); // lokasi
            $this->updateSort($this->mitra_id); // mitra_id
            $this->updateSort($this->peserta_terdaftar); // peserta_terdaftar
    ...
    protected function resetCmd(): void
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->event_id->setSort("");
                $this->nama_event->setSort("");
                $this->deskripsi->setSort("");
                $this->tanggal_mulai->setSort("");
                $this->tanggal_selesai->setSort("");
                $this->lokasi->setSort("");
                $this->mitra_id->setSort("");
                $this->peserta_terdaftar->setSort("");
            }
    ...
    public function renderListOptions(): void
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($this->security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"event\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $this->language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($this->security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"event\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-ask=\"1\" data-btn=\"SaveBtn\">" . $this->language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($this->security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"event\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("CopyLink") . "</a>";
                } else {
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"feventlist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"feventlist\" data-key=\"" . $this->keyToJson(true) .
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->event_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
			$item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"event\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "event_id");
            $this->createColumnOption($option, "nama_event");
            $this->createColumnOption($option, "deskripsi");
            $this->createColumnOption($option, "tanggal_mulai");
            $this->createColumnOption($option, "tanggal_selesai");
            $this->createColumnOption($option, "lokasi");
            $this->createColumnOption($option, "mitra_id");
            $this->createColumnOption($option, "peserta_terdaftar");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"feventsrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"feventsrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="feventlist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    public function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_event", "data-rowtype" => RowType::ADD]);
...
// Load basic search values
    ...
    public function loadRowValues(array|bool|null $row = null): void
    {
        ...
        $this->nama_event->setDbValue($row['nama_event']);
        ...
        $this->peserta_terdaftar->setDbValue($row['peserta_terdaftar']);
    }
    ...
    protected function newRow(): array
    {
        ...
        $row['nama_event'] = $this->nama_event->DefaultValue;
        ...
        $row['peserta_terdaftar'] = $this->peserta_terdaftar->DefaultValue;
        return $row;
    }
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // nama_event
            $this->nama_event->ViewValue = $this->nama_event->CurrentValue;
            ...
            // peserta_terdaftar
            $this->peserta_terdaftar->ViewValue = $this->peserta_terdaftar->CurrentValue;
            $this->peserta_terdaftar->ViewValue = FormatNumber($this->peserta_terdaftar->ViewValue, $this->peserta_terdaftar->formatPattern());
            ...
            // nama_event
            $this->nama_event->HrefValue = "";
            $this->nama_event->TooltipValue = "";
            ...
            // peserta_terdaftar
            $this->peserta_terdaftar->HrefValue = "";
            $this->peserta_terdaftar->TooltipValue = "";
        }
	...
	// Begin of add Search Panel Status by Masino Sinaga, October 13, 2024
		...
		if (ReadCookie('event_searchpanel') == 'notactive' || ReadCookie('event_searchpanel') == "") {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"feventsrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		} elseif (ReadCookie('event_searchpanel') == 'active') {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle active\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"feventsrch\" aria-pressed=\"true\">" . $this->language->phrase("SearchLink") . "</a>";
		} else {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"feventsrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		}
        $item->Visible = true;
