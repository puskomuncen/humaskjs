 ...
 * Page class
 */
class LanguagesList extends Languages
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "LanguagesList";
    ...
    public string $FormName = "flanguageslist";

    // CSS class/style
    public string $CurrentPageName = "languageslist";
    ...
    public function setVisibility(): void
    {
        ...
        $this->Site_Logo->Visible = false;
        $this->Site_Title->Visible = false;
        $this->Default_Thousands_Separator->Visible = false;
        $this->Default_Decimal_Point->Visible = false;
        $this->Default_Currency_Symbol->Visible = false;
        $this->Default_Money_Thousands_Separator->Visible = false;
        $this->Default_Money_Decimal_Point->Visible = false;
        $this->Terms_And_Condition_Text->Visible = false;
        $this->Announcement_Text->Visible = false;
        $this->About_Text->Visible = false;
    }
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        // Initialize URLs
        $this->AddUrl = "languagesadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "languagesdelete";
        $this->MultiUpdateUrl = "languagesupdate";
...
// Is modal
		...
		if (ReadCookie('languages_searchpanel') == 'notactive' || ReadCookie('languages_searchpanel') == "") {
			RemoveClass($this->SearchPanelClass, "show");
			AppendClass($this->SearchPanelClass, "collapse");
		} elseif (ReadCookie('languages_searchpanel') == 'active') {
    ...
    public function getFilterList(): string
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("flanguagessrch");
        }
        $filterList = Concat($filterList, $this->Language_Code->AdvancedSearch->toJson(), ","); // Field Language_Code
        $filterList = Concat($filterList, $this->Language_Name->AdvancedSearch->toJson(), ","); // Field Language_Name
        $filterList = Concat($filterList, $this->Default->AdvancedSearch->toJson(), ","); // Field Default
        $filterList = Concat($filterList, $this->Site_Logo->AdvancedSearch->toJson(), ","); // Field Site_Logo
        $filterList = Concat($filterList, $this->Site_Title->AdvancedSearch->toJson(), ","); // Field Site_Title
        $filterList = Concat($filterList, $this->Default_Thousands_Separator->AdvancedSearch->toJson(), ","); // Field Default_Thousands_Separator
        $filterList = Concat($filterList, $this->Default_Decimal_Point->AdvancedSearch->toJson(), ","); // Field Default_Decimal_Point
        $filterList = Concat($filterList, $this->Default_Currency_Symbol->AdvancedSearch->toJson(), ","); // Field Default_Currency_Symbol
        $filterList = Concat($filterList, $this->Default_Money_Thousands_Separator->AdvancedSearch->toJson(), ","); // Field Default_Money_Thousands_Separator
        $filterList = Concat($filterList, $this->Default_Money_Decimal_Point->AdvancedSearch->toJson(), ","); // Field Default_Money_Decimal_Point
        $filterList = Concat($filterList, $this->Terms_And_Condition_Text->AdvancedSearch->toJson(), ","); // Field Terms_And_Condition_Text
        $filterList = Concat($filterList, $this->Announcement_Text->AdvancedSearch->toJson(), ","); // Field Announcement_Text
        $filterList = Concat($filterList, $this->About_Text->AdvancedSearch->toJson(), ","); // Field About_Text
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("flanguagessrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        // Field Default
        $this->Default->AdvancedSearch->SearchValue = $filter["x_Default"] ?? "";
        $this->Default->AdvancedSearch->SearchOperator = $filter["z_Default"] ?? "";
        $this->Default->AdvancedSearch->SearchCondition = $filter["v_Default"] ?? "";
        $this->Default->AdvancedSearch->SearchValue2 = $filter["y_Default"] ?? "";
        $this->Default->AdvancedSearch->SearchOperator2 = $filter["w_Default"] ?? "";
        $this->Default->AdvancedSearch->save();
    ...
    public function basicSearchWhere(bool $default = false): string
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->Language_Code;
        $searchFlds[] = &$this->Language_Name;
        $searchFlds[] = &$this->Site_Logo;
        $searchFlds[] = &$this->Site_Title;
        $searchFlds[] = &$this->Default_Thousands_Separator;
        $searchFlds[] = &$this->Default_Decimal_Point;
        $searchFlds[] = &$this->Default_Currency_Symbol;
        $searchFlds[] = &$this->Default_Money_Thousands_Separator;
        $searchFlds[] = &$this->Default_Money_Decimal_Point;
        $searchFlds[] = &$this->Terms_And_Condition_Text;
        $searchFlds[] = &$this->Announcement_Text;
        $searchFlds[] = &$this->About_Text;
    ...
    protected function setupSortOrder(): void
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->Default); // Default
    ...
    protected function resetCmd(): void
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->Default->setSort("");
                $this->Site_Logo->setSort("");
                $this->Site_Title->setSort("");
                $this->Default_Thousands_Separator->setSort("");
                $this->Default_Decimal_Point->setSort("");
                $this->Default_Currency_Symbol->setSort("");
                $this->Default_Money_Thousands_Separator->setSort("");
                $this->Default_Money_Decimal_Point->setSort("");
                $this->Terms_And_Condition_Text->setSort("");
                $this->Announcement_Text->setSort("");
                $this->About_Text->setSort("");
            }
    ...
    public function renderListOptions(): void
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($this->security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"languages\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $this->language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($this->security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"languages\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-ask=\"1\" data-btn=\"SaveBtn\">" . $this->language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($this->security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"languages\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("CopyLink") . "</a>";
                } else {
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"flanguageslist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"flanguageslist\" data-key=\"" . $this->keyToJson(true) .
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->Language_Code->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
			$item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"languages\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "Language_Code");
            $this->createColumnOption($option, "Language_Name");
            $this->createColumnOption($option, "Default");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"flanguagessrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"flanguagessrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="flanguageslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    public function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_languages", "data-rowtype" => RowType::ADD]);
...
// Load basic search values
	...
	// Begin of add Search Panel Status by Masino Sinaga, October 13, 2024
		...
		if (ReadCookie('languages_searchpanel') == 'notactive' || ReadCookie('languages_searchpanel') == "") {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"flanguagessrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		} elseif (ReadCookie('languages_searchpanel') == 'active') {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle active\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"flanguagessrch\" aria-pressed=\"true\">" . $this->language->phrase("SearchLink") . "</a>";
		} else {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"flanguagessrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		}
        $item->Visible = true;
