 ...
 * Page class
 */
class InteraksiEdit extends Interaksi
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "InteraksiEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "interaksiedit";
...
// Is modal
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("interaksi_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->interaksi_id->setQueryStringValue($keyValue);
                $this->interaksi_id->setOldValue($this->interaksi_id->QueryStringValue);
            } elseif (Post("interaksi_id") !== null) {
                $this->interaksi_id->setFormValue(Post("interaksi_id"));
                $this->interaksi_id->setOldValue($this->interaksi_id->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("interaksi_id") ?? Route("interaksi_id")) !== null) {
                    $this->interaksi_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->interaksi_id->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("interaksilist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "interaksilist") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "interaksilist") {
                            ...
                            $returnUrl = "interaksilist"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
...
// Get upload files
    ...
    protected function loadFormValues(): void
    {
        ...
        // Check field name 'interaksi_id' before field var 'x_interaksi_id'
        $val = $this->getFormValue("interaksi_id", null) ?? $this->getFormValue("x_interaksi_id", null);
        if (!$this->interaksi_id->IsDetailKey) {
            $this->interaksi_id->setFormValue($val);
        }
        ...
        // Check field name 'jenis_interaksi' before field var 'x_jenis_interaksi'
        $val = $this->getFormValue("jenis_interaksi", null) ?? $this->getFormValue("x_jenis_interaksi", null);
        if (!$this->jenis_interaksi->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->jenis_interaksi->Visible = false; // Disable update for API request
            } else {
                $this->jenis_interaksi->setFormValue($val);
            }
        }
        ...
        // Check field name 'tanggal_interaksi' before field var 'x_tanggal_interaksi'
        $val = $this->getFormValue("tanggal_interaksi", null) ?? $this->getFormValue("x_tanggal_interaksi", null);
        if (!$this->tanggal_interaksi->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->tanggal_interaksi->Visible = false; // Disable update for API request
            } else {
                $this->tanggal_interaksi->setFormValue($val, true, $validate);
            }
            $this->tanggal_interaksi->CurrentValue = UnformatDateTime($this->tanggal_interaksi->CurrentValue, $this->tanggal_interaksi->formatPattern());
        }
    }
    ...
    {
        $this->interaksi_id->CurrentValue = $this->interaksi_id->FormValue;
        $this->mitra_id->CurrentValue = $this->mitra_id->FormValue;
        $this->user_id->CurrentValue = $this->user_id->FormValue;
        $this->jenis_interaksi->CurrentValue = $this->jenis_interaksi->FormValue;
        $this->catatan->CurrentValue = $this->catatan->FormValue;
        $this->tanggal_interaksi->CurrentValue = $this->tanggal_interaksi->FormValue;
        $this->tanggal_interaksi->CurrentValue = UnformatDateTime($this->tanggal_interaksi->CurrentValue, $this->tanggal_interaksi->formatPattern());
    }
    ...
    public function renderRow(): void
    {
        ...
        // interaksi_id
        $this->interaksi_id->RowCssClass = "row";
        ...
        // jenis_interaksi
        $this->jenis_interaksi->RowCssClass = "row";

        // catatan
        $this->catatan->RowCssClass = "row";

        // tanggal_interaksi
        $this->tanggal_interaksi->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // interaksi_id
            $this->interaksi_id->setupEditAttributes();
            $this->interaksi_id->EditValue = $this->interaksi_id->CurrentValue;
            ...
            // jenis_interaksi
            $this->jenis_interaksi->EditValue = $this->jenis_interaksi->options(false);
            $this->jenis_interaksi->PlaceHolder = RemoveHtml($this->jenis_interaksi->caption());
            ...
            // tanggal_interaksi
            $this->tanggal_interaksi->setupEditAttributes();
            $this->tanggal_interaksi->EditValue = FormatDateTime($this->tanggal_interaksi->CurrentValue, $this->tanggal_interaksi->formatPattern());
            $this->tanggal_interaksi->PlaceHolder = RemoveHtml($this->tanggal_interaksi->caption());
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            if ($this->interaksi_id->Visible && $this->interaksi_id->Required) {
                if (!$this->interaksi_id->IsDetailKey && IsEmpty($this->interaksi_id->FormValue)) {
                    $this->interaksi_id->addErrorMessage(str_replace("%s", $this->interaksi_id->caption(), $this->interaksi_id->RequiredErrorMessage));
                }
            }
            ...
            if ($this->jenis_interaksi->Visible && $this->jenis_interaksi->Required) {
                if ($this->jenis_interaksi->FormValue == "") {
                    $this->jenis_interaksi->addErrorMessage(str_replace("%s", $this->jenis_interaksi->caption(), $this->jenis_interaksi->RequiredErrorMessage));
                }
            }
            ...
            if ($this->tanggal_interaksi->Visible && $this->tanggal_interaksi->Required) {
                if (!$this->tanggal_interaksi->IsDetailKey && IsEmpty($this->tanggal_interaksi->FormValue)) {
                    $this->tanggal_interaksi->addErrorMessage(str_replace("%s", $this->tanggal_interaksi->caption(), $this->tanggal_interaksi->RequiredErrorMessage));
                }
            }
            if (!CheckDate($this->tanggal_interaksi->FormValue, $this->tanggal_interaksi->formatPattern())) {
                $this->tanggal_interaksi->addErrorMessage($this->tanggal_interaksi->getErrorMessage(false));
            }
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // jenis_interaksi
        $this->jenis_interaksi->setDbValueDef($newRow, $this->jenis_interaksi->CurrentValue, $this->jenis_interaksi->ReadOnly);

        // catatan
        $this->catatan->setDbValueDef($newRow, $this->catatan->CurrentValue, $this->catatan->ReadOnly);

        // tanggal_interaksi
        $this->tanggal_interaksi->setDbValueDef($newRow, UnFormatDateTime($this->tanggal_interaksi->CurrentValue, $this->tanggal_interaksi->formatPattern()), $this->tanggal_interaksi->ReadOnly);
        return $newRow;
    }
    ...
    protected function restoreEditFormFromRow(array $row): void
    {
        ...
        if (isset($row['jenis_interaksi'])) { // jenis_interaksi
            $this->jenis_interaksi->CurrentValue = $row['jenis_interaksi'];
        }
        if (isset($row['catatan'])) { // catatan
            $this->catatan->CurrentValue = $row['catatan'];
        }
        if (isset($row['tanggal_interaksi'])) { // tanggal_interaksi
            $this->tanggal_interaksi->CurrentValue = $row['tanggal_interaksi'];
        }
    }
    ...
    protected function setupBreadcrumb(): void
    {
        ...
        $url = CurrentUrl();
        $breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("interaksilist"), "", $this->TableVar, true);
        $pageId = "edit";
