 ...
 * Page class
 */
class PublikasiList extends Publikasi
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "PublikasiList";
    ...
    public string $FormName = "fpublikasilist";

    // CSS class/style
    public string $CurrentPageName = "publikasilist";
    ...
    {
        $this->publikasi_id->setVisibility();
        $this->judul->setVisibility();
        $this->konten->setVisibility();
        $this->tanggal_publikasi->setVisibility();
        $this->jenis_media->setVisibility();
        $this->gambar_path->setVisibility();
        $this->penulis_id->setVisibility();
    }
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        // Table object (publikasi)
        if (!isset($GLOBALS["publikasi"]) || $GLOBALS["publikasi"]::class == PROJECT_NAMESPACE . "publikasi") {
            $GLOBALS["publikasi"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "publikasiadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "publikasidelete";
        $this->MultiUpdateUrl = "publikasiupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'publikasi');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "publikasiview"); // If View page, no primary button
                } else { // List page
    ...
    protected function hideFieldsForAddEdit(): void
    {
        if ($this->isAdd() || $this->isCopy() || $this->isGridAdd()) {
            $this->publikasi_id->Visible = false;
        }
    }
...
// Is modal
        ...
        // Set up lookup cache
        $this->setupLookupOptions($this->jenis_media);

        // Update form name to avoid conflict
        if ($this->IsModal) {
            $this->FormName = "fpublikasigrid";
        }
		...
		if (ReadCookie('publikasi_searchpanel') == 'notactive' || ReadCookie('publikasi_searchpanel') == "") {
			RemoveClass($this->SearchPanelClass, "show");
			AppendClass($this->SearchPanelClass, "collapse");
		} elseif (ReadCookie('publikasi_searchpanel') == 'active') {
    ...
    public function getFilterList(): string
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fpublikasisrch");
        }
        $filterList = Concat($filterList, $this->publikasi_id->AdvancedSearch->toJson(), ","); // Field publikasi_id
        $filterList = Concat($filterList, $this->judul->AdvancedSearch->toJson(), ","); // Field judul
        $filterList = Concat($filterList, $this->konten->AdvancedSearch->toJson(), ","); // Field konten
        $filterList = Concat($filterList, $this->tanggal_publikasi->AdvancedSearch->toJson(), ","); // Field tanggal_publikasi
        $filterList = Concat($filterList, $this->jenis_media->AdvancedSearch->toJson(), ","); // Field jenis_media
        $filterList = Concat($filterList, $this->gambar_path->AdvancedSearch->toJson(), ","); // Field gambar_path
        $filterList = Concat($filterList, $this->penulis_id->AdvancedSearch->toJson(), ","); // Field penulis_id
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fpublikasisrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        // Field publikasi_id
        $this->publikasi_id->AdvancedSearch->SearchValue = $filter["x_publikasi_id"] ?? "";
        $this->publikasi_id->AdvancedSearch->SearchOperator = $filter["z_publikasi_id"] ?? "";
        $this->publikasi_id->AdvancedSearch->SearchCondition = $filter["v_publikasi_id"] ?? "";
        $this->publikasi_id->AdvancedSearch->SearchValue2 = $filter["y_publikasi_id"] ?? "";
        $this->publikasi_id->AdvancedSearch->SearchOperator2 = $filter["w_publikasi_id"] ?? "";
        $this->publikasi_id->AdvancedSearch->save();

        // Field judul
        $this->judul->AdvancedSearch->SearchValue = $filter["x_judul"] ?? "";
        $this->judul->AdvancedSearch->SearchOperator = $filter["z_judul"] ?? "";
        $this->judul->AdvancedSearch->SearchCondition = $filter["v_judul"] ?? "";
        $this->judul->AdvancedSearch->SearchValue2 = $filter["y_judul"] ?? "";
        $this->judul->AdvancedSearch->SearchOperator2 = $filter["w_judul"] ?? "";
        $this->judul->AdvancedSearch->save();

        // Field konten
        $this->konten->AdvancedSearch->SearchValue = $filter["x_konten"] ?? "";
        $this->konten->AdvancedSearch->SearchOperator = $filter["z_konten"] ?? "";
        $this->konten->AdvancedSearch->SearchCondition = $filter["v_konten"] ?? "";
        $this->konten->AdvancedSearch->SearchValue2 = $filter["y_konten"] ?? "";
        $this->konten->AdvancedSearch->SearchOperator2 = $filter["w_konten"] ?? "";
        $this->konten->AdvancedSearch->save();

        // Field tanggal_publikasi
        $this->tanggal_publikasi->AdvancedSearch->SearchValue = $filter["x_tanggal_publikasi"] ?? "";
        $this->tanggal_publikasi->AdvancedSearch->SearchOperator = $filter["z_tanggal_publikasi"] ?? "";
        $this->tanggal_publikasi->AdvancedSearch->SearchCondition = $filter["v_tanggal_publikasi"] ?? "";
        $this->tanggal_publikasi->AdvancedSearch->SearchValue2 = $filter["y_tanggal_publikasi"] ?? "";
        $this->tanggal_publikasi->AdvancedSearch->SearchOperator2 = $filter["w_tanggal_publikasi"] ?? "";
        $this->tanggal_publikasi->AdvancedSearch->save();

        // Field jenis_media
        $this->jenis_media->AdvancedSearch->SearchValue = $filter["x_jenis_media"] ?? "";
        $this->jenis_media->AdvancedSearch->SearchOperator = $filter["z_jenis_media"] ?? "";
        $this->jenis_media->AdvancedSearch->SearchCondition = $filter["v_jenis_media"] ?? "";
        $this->jenis_media->AdvancedSearch->SearchValue2 = $filter["y_jenis_media"] ?? "";
        $this->jenis_media->AdvancedSearch->SearchOperator2 = $filter["w_jenis_media"] ?? "";
        $this->jenis_media->AdvancedSearch->save();

        // Field gambar_path
        $this->gambar_path->AdvancedSearch->SearchValue = $filter["x_gambar_path"] ?? "";
        $this->gambar_path->AdvancedSearch->SearchOperator = $filter["z_gambar_path"] ?? "";
        $this->gambar_path->AdvancedSearch->SearchCondition = $filter["v_gambar_path"] ?? "";
        $this->gambar_path->AdvancedSearch->SearchValue2 = $filter["y_gambar_path"] ?? "";
        $this->gambar_path->AdvancedSearch->SearchOperator2 = $filter["w_gambar_path"] ?? "";
        $this->gambar_path->AdvancedSearch->save();

        // Field penulis_id
        $this->penulis_id->AdvancedSearch->SearchValue = $filter["x_penulis_id"] ?? "";
        $this->penulis_id->AdvancedSearch->SearchOperator = $filter["z_penulis_id"] ?? "";
        $this->penulis_id->AdvancedSearch->SearchCondition = $filter["v_penulis_id"] ?? "";
        $this->penulis_id->AdvancedSearch->SearchValue2 = $filter["y_penulis_id"] ?? "";
        $this->penulis_id->AdvancedSearch->SearchOperator2 = $filter["w_penulis_id"] ?? "";
        $this->penulis_id->AdvancedSearch->save();
    ...
    public function basicSearchWhere(bool $default = false): string
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->judul;
        $searchFlds[] = &$this->konten;
        $searchFlds[] = &$this->gambar_path;
    ...
    protected function setupSortOrder(): void
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->publikasi_id); // publikasi_id
            $this->updateSort($this->judul); // judul
            $this->updateSort($this->konten); // konten
            $this->updateSort($this->tanggal_publikasi); // tanggal_publikasi
            $this->updateSort($this->jenis_media); // jenis_media
            $this->updateSort($this->gambar_path); // gambar_path
            $this->updateSort($this->penulis_id); // penulis_id
    ...
    protected function resetCmd(): void
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->publikasi_id->setSort("");
                $this->judul->setSort("");
                $this->konten->setSort("");
                $this->tanggal_publikasi->setSort("");
                $this->jenis_media->setSort("");
                $this->gambar_path->setSort("");
                $this->penulis_id->setSort("");
            }
    ...
    public function renderListOptions(): void
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($this->security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"publikasi\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $this->language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($this->security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"publikasi\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-ask=\"1\" data-btn=\"SaveBtn\">" . $this->language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($this->security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"publikasi\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("CopyLink") . "</a>";
                } else {
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fpublikasilist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fpublikasilist\" data-key=\"" . $this->keyToJson(true) .
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->publikasi_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
			$item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"publikasi\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "publikasi_id");
            $this->createColumnOption($option, "judul");
            $this->createColumnOption($option, "konten");
            $this->createColumnOption($option, "tanggal_publikasi");
            $this->createColumnOption($option, "jenis_media");
            $this->createColumnOption($option, "gambar_path");
            $this->createColumnOption($option, "penulis_id");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fpublikasisrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fpublikasisrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fpublikasilist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    public function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_publikasi", "data-rowtype" => RowType::ADD]);
...
// Load basic search values
    ...
    {
        $row = [];
        $row['publikasi_id'] = $this->publikasi_id->DefaultValue;
        $row['judul'] = $this->judul->DefaultValue;
        $row['konten'] = $this->konten->DefaultValue;
        $row['tanggal_publikasi'] = $this->tanggal_publikasi->DefaultValue;
        $row['jenis_media'] = $this->jenis_media->DefaultValue;
        $row['gambar_path'] = $this->gambar_path->DefaultValue;
        $row['penulis_id'] = $this->penulis_id->DefaultValue;
        return $row;
    }
	...
	// Begin of add Search Panel Status by Masino Sinaga, October 13, 2024
		...
		if (ReadCookie('publikasi_searchpanel') == 'notactive' || ReadCookie('publikasi_searchpanel') == "") {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fpublikasisrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		} elseif (ReadCookie('publikasi_searchpanel') == 'active') {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle active\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fpublikasisrch\" aria-pressed=\"true\">" . $this->language->phrase("SearchLink") . "</a>";
		} else {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fpublikasisrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		}
        $item->Visible = true;
