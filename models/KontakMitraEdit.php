 ...
 * Page class
 */
class KontakMitraEdit extends KontakMitra
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "KontakMitraEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "kontakmitraedit";
...
// Is modal
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("kontak_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->kontak_id->setQueryStringValue($keyValue);
                $this->kontak_id->setOldValue($this->kontak_id->QueryStringValue);
            } elseif (Post("kontak_id") !== null) {
                $this->kontak_id->setFormValue(Post("kontak_id"));
                $this->kontak_id->setOldValue($this->kontak_id->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("kontak_id") ?? Route("kontak_id")) !== null) {
                    $this->kontak_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->kontak_id->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("kontakmitralist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "kontakmitralist") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "kontakmitralist") {
                            ...
                            $returnUrl = "kontakmitralist"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
...
// Get upload files
    ...
    protected function loadFormValues(): void
    {
        ...
        // Check field name 'kontak_id' before field var 'x_kontak_id'
        $val = $this->getFormValue("kontak_id", null) ?? $this->getFormValue("x_kontak_id", null);
        if (!$this->kontak_id->IsDetailKey) {
            $this->kontak_id->setFormValue($val);
        }
        ...
        // Check field name 'nama_kontak' before field var 'x_nama_kontak'
        $val = $this->getFormValue("nama_kontak", null) ?? $this->getFormValue("x_nama_kontak", null);
        if (!$this->nama_kontak->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->nama_kontak->Visible = false; // Disable update for API request
            } else {
                $this->nama_kontak->setFormValue($val);
            }
        }

        // Check field name 'jabatan' before field var 'x_jabatan'
        $val = $this->getFormValue("jabatan", null) ?? $this->getFormValue("x_jabatan", null);
    ...
    {
        $this->kontak_id->CurrentValue = $this->kontak_id->FormValue;
        $this->mitra_id->CurrentValue = $this->mitra_id->FormValue;
        $this->nama_kontak->CurrentValue = $this->nama_kontak->FormValue;
    ...
    public function renderRow(): void
    {
        ...
        // kontak_id
        $this->kontak_id->RowCssClass = "row";

        // mitra_id
        $this->mitra_id->RowCssClass = "row";

        // nama_kontak
        $this->nama_kontak->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // kontak_id
            $this->kontak_id->setupEditAttributes();
            $this->kontak_id->EditValue = $this->kontak_id->CurrentValue;
            ...
            // nama_kontak
            $this->nama_kontak->setupEditAttributes();
            $this->nama_kontak->EditValue = !$this->nama_kontak->Raw ? HtmlDecode($this->nama_kontak->CurrentValue) : $this->nama_kontak->CurrentValue;
            $this->nama_kontak->PlaceHolder = RemoveHtml($this->nama_kontak->caption());

            // jabatan
            $this->jabatan->setupEditAttributes();
            $this->jabatan->EditValue = !$this->jabatan->Raw ? HtmlDecode($this->jabatan->CurrentValue) : $this->jabatan->CurrentValue;
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            if ($this->kontak_id->Visible && $this->kontak_id->Required) {
                if (!$this->kontak_id->IsDetailKey && IsEmpty($this->kontak_id->FormValue)) {
                    $this->kontak_id->addErrorMessage(str_replace("%s", $this->kontak_id->caption(), $this->kontak_id->RequiredErrorMessage));
                }
            }
            ...
            if ($this->nama_kontak->Visible && $this->nama_kontak->Required) {
                if (!$this->nama_kontak->IsDetailKey && IsEmpty($this->nama_kontak->FormValue)) {
                    $this->nama_kontak->addErrorMessage(str_replace("%s", $this->nama_kontak->caption(), $this->nama_kontak->RequiredErrorMessage));
                }
            }
            if ($this->jabatan->Visible && $this->jabatan->Required) {
                if (!$this->jabatan->IsDetailKey && IsEmpty($this->jabatan->FormValue)) {
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // nama_kontak
        $this->nama_kontak->setDbValueDef($newRow, $this->nama_kontak->CurrentValue, $this->nama_kontak->ReadOnly);

        // jabatan
        $this->jabatan->setDbValueDef($newRow, $this->jabatan->CurrentValue, $this->jabatan->ReadOnly);

        // email
        $this->email->setDbValueDef($newRow, $this->email->CurrentValue, $this->email->ReadOnly);

        // telepon
        $this->telepon->setDbValueDef($newRow, $this->telepon->CurrentValue, $this->telepon->ReadOnly);
        return $newRow;
    }
    ...
    protected function restoreEditFormFromRow(array $row): void
    {
        ...
        if (isset($row['nama_kontak'])) { // nama_kontak
            $this->nama_kontak->CurrentValue = $row['nama_kontak'];
        }
        if (isset($row['jabatan'])) { // jabatan
            $this->jabatan->CurrentValue = $row['jabatan'];
        }
        if (isset($row['email'])) { // email
            $this->email->CurrentValue = $row['email'];
        }
        if (isset($row['telepon'])) { // telepon
            $this->telepon->CurrentValue = $row['telepon'];
        }
    }
    ...
    protected function setupBreadcrumb(): void
    {
        ...
        $url = CurrentUrl();
        $breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("kontakmitralist"), "", $this->TableVar, true);
        $pageId = "edit";
