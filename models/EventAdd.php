...
class EventAdd extends Event
{
    ...
    // Page object name
    public string $PageObjName = "EventAdd";
    ...
    // CSS class/style
    public string $CurrentPageName = "eventadd";
...
// Is modal
        ...
        if (IsApi()) {
            ...
            if (($keyValue = Get("event_id") ?? Route("event_id")) !== null) {
                $this->event_id->setQueryStringValue($keyValue);
            }
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$oldRow) { // Record not loaded
                    ...
                    $this->terminate("eventlist"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                if ($this->addRow($oldRow)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "eventlist") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "eventview") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "eventlist") {
                            ...
                            $returnUrl = "eventlist"; // Return list page content
                        }
                    }
...
// Get upload files
    ...
    {
        $this->peserta_terdaftar->DefaultValue = $this->peserta_terdaftar->getDefault(); // PHP
        $this->peserta_terdaftar->OldValue = $this->peserta_terdaftar->DefaultValue;
    }
    ...
    protected function loadFormValues(): void
    {
        ...
        // Check field name 'nama_event' before field var 'x_nama_event'
        $val = $this->getFormValue("nama_event", null) ?? $this->getFormValue("x_nama_event", null);
        if (!$this->nama_event->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->nama_event->Visible = false; // Disable update for API request
            } else {
                $this->nama_event->setFormValue($val);
            }
        }
        ...
        $val = $this->getFormValue("lokasi", null) ?? $this->getFormValue("x_lokasi", null);
        ...
        // Check field name 'peserta_terdaftar' before field var 'x_peserta_terdaftar'
        $val = $this->getFormValue("peserta_terdaftar", null) ?? $this->getFormValue("x_peserta_terdaftar", null);
        if (!$this->peserta_terdaftar->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->peserta_terdaftar->Visible = false; // Disable update for API request
            } else {
                $this->peserta_terdaftar->setFormValue($val, true, $validate);
            }
        }

        // Check field name 'event_id' first before field var 'x_event_id'
        $val = $this->hasFormValue("event_id") ? $this->getFormValue("event_id") : $this->getFormValue("x_event_id");
    }

    // Restore form values
    public function restoreFormValues(): void
    {
        $this->nama_event->CurrentValue = $this->nama_event->FormValue;
        ...
        $this->peserta_terdaftar->CurrentValue = $this->peserta_terdaftar->FormValue;
    }
    ...
    public function renderRow(): void
    {
        ...
        // nama_event
        $this->nama_event->RowCssClass = "row";
        ...
        // peserta_terdaftar
        $this->peserta_terdaftar->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // nama_event
            $this->nama_event->setupEditAttributes();
            $this->nama_event->EditValue = !$this->nama_event->Raw ? HtmlDecode($this->nama_event->CurrentValue) : $this->nama_event->CurrentValue;
            $this->nama_event->PlaceHolder = RemoveHtml($this->nama_event->caption());
            ...
            $this->lokasi->EditValue = !$this->lokasi->Raw ? HtmlDecode($this->lokasi->CurrentValue) : $this->lokasi->CurrentValue;
            ...
            // peserta_terdaftar
            $this->peserta_terdaftar->setupEditAttributes();
            $this->peserta_terdaftar->EditValue = $this->peserta_terdaftar->CurrentValue;
            $this->peserta_terdaftar->PlaceHolder = RemoveHtml($this->peserta_terdaftar->caption());
            if (strval($this->peserta_terdaftar->EditValue) != "" && is_numeric($this->peserta_terdaftar->EditValue)) {
                $this->peserta_terdaftar->EditValue = FormatNumber($this->peserta_terdaftar->EditValue, null);
            }
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            if ($this->nama_event->Visible && $this->nama_event->Required) {
                if (!$this->nama_event->IsDetailKey && IsEmpty($this->nama_event->FormValue)) {
                    $this->nama_event->addErrorMessage(str_replace("%s", $this->nama_event->caption(), $this->nama_event->RequiredErrorMessage));
                }
            }
            ...
            if ($this->lokasi->Visible && $this->lokasi->Required) {
                if (!$this->lokasi->IsDetailKey && IsEmpty($this->lokasi->FormValue)) {
            ...
            if ($this->peserta_terdaftar->Visible && $this->peserta_terdaftar->Required) {
                if (!$this->peserta_terdaftar->IsDetailKey && IsEmpty($this->peserta_terdaftar->FormValue)) {
                    $this->peserta_terdaftar->addErrorMessage(str_replace("%s", $this->peserta_terdaftar->caption(), $this->peserta_terdaftar->RequiredErrorMessage));
                }
            }
            if (!CheckInteger($this->peserta_terdaftar->FormValue)) {
                $this->peserta_terdaftar->addErrorMessage($this->peserta_terdaftar->getErrorMessage(false));
            }
    ...
    protected function getAddRow(): array
    {
        ...
        // nama_event
        $this->nama_event->setDbValueDef($newRow, $this->nama_event->CurrentValue, false);

        // deskripsi
        $this->deskripsi->setDbValueDef($newRow, $this->deskripsi->CurrentValue, false);

        // tanggal_mulai
        $this->tanggal_mulai->setDbValueDef($newRow, UnFormatDateTime($this->tanggal_mulai->CurrentValue, $this->tanggal_mulai->formatPattern()), false);

        // tanggal_selesai
        $this->tanggal_selesai->setDbValueDef($newRow, UnFormatDateTime($this->tanggal_selesai->CurrentValue, $this->tanggal_selesai->formatPattern()), false);

        // lokasi
        $this->lokasi->setDbValueDef($newRow, $this->lokasi->CurrentValue, false);

        // mitra_id
        $this->mitra_id->setDbValueDef($newRow, $this->mitra_id->CurrentValue, false);

        // peserta_terdaftar
        $this->peserta_terdaftar->setDbValueDef($newRow, $this->peserta_terdaftar->CurrentValue, strval($this->peserta_terdaftar->CurrentValue) == "");
        return $newRow;
    }
    ...
    protected function setupBreadcrumb(): void
    {
        ...
        $url = CurrentUrl();
        $breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("eventlist"), "", $this->TableVar, true);
