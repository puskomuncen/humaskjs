...
<form name="fhelp_categoriesedit" id="fhelp_categoriesedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script<?= Nonce() ?>>
var currentTable = <?= json_encode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { help_categories: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fhelp_categoriesedit;
...
<?php if ($Page->Category_ID->Visible) { // Category_ID ?>
    <div id="r_Category_ID"<?= $Page->Category_ID->rowAttributes() ?>>
        <label id="elh_help_categories_Category_ID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Category_ID->caption() ?><?= $Page->Category_ID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->Category_ID->cellAttributes() ?>>
<span id="el_help_categories_Category_ID">
<span<?= $Page->Category_ID->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->Category_ID->getDisplayValue($Page->Category_ID->getEditValue()))) ?>"></span>
<input type="hidden" data-table="help_categories" data-field="x_Category_ID" data-hidden="1" name="x_Category_ID" id="x_Category_ID" value="<?= HtmlEncode($Page->Category_ID->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_help_categories__Language">
    <select
        ...
        data-select2-id="fhelp_categoriesedit_x__Language"
        <?php } ?>
...
<?php if (!$Page->_Language->IsNativeSelect) { ?>
<script<?= Nonce() ?>>
loadjs.ready("fhelp_categoriesedit", function() {
    var options = { name: "x__Language", selectId: "fhelp_categoriesedit_x__Language" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fhelp_categoriesedit.lists._Language?.lookupOptions.length) {
        options.data = { id: "x__Language", form: "fhelp_categoriesedit" };
    } else {
        options.ajax = { id: "x__Language", form: "fhelp_categoriesedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php
    if (in_array("help", explode(",", $Page->getCurrentDetailTable())) && $help->DetailEdit) {
?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fhelp_categoriesedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fhelp_categoriesedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
...
<script>
loadjs.ready("head", function() { $("#fhelp_categoriesedit:first *:input[type!=hidden]:first").focus(),$("input").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("select").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("radio").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()})});
</script>
<?php } ?>
<script<?= Nonce() ?>>
