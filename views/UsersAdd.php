...
<div class="input-group" id="ig__Password">
    <input type="password" autocomplete="new-password" data-password-strength="pst__Password" data-table="users" data-field="x__Password" name="x__Password" id="x__Password" size="30" maxlength="255" placeholder="<?= HtmlEncode($Page->_Password->getPlaceHolder()) ?>"<?= $Page->_Password->editAttributes() ?> aria-describedby="x__Password_help">
...
<span id="el_users_UserLevel">
    <select
        ...
        data-select2-id="fusersadd_x_UserLevel"
        <?php } ?>
        data-table="users"
...
loadjs.ready("fusersadd", function() {
    var options = { name: "x_UserLevel", selectId: "fusersadd_x_UserLevel" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fusersadd.lists.UserLevel?.lookupOptions.length) {
        options.data = { id: "x_UserLevel", form: "fusersadd" };
    } else {
        options.ajax = { id: "x_UserLevel", form: "fusersadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php } ?>
<script<?= Nonce() ?>>
loadjs.ready(['fusersadd', 'jqueryinputmask'], function() {
	options = {
		'jitMasking': false,
		'removeMaskOnSubmit': true
	};
	ew.createjQueryInputMask("fusersadd", "x_BirthDate", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
<span id="el_users_ReportsTo">
    <select
        ...
        data-select2-id="fusersadd_x_ReportsTo"
        <?php } ?>
        data-table="users"
...
<script<?= Nonce() ?>>
loadjs.ready("fusersadd", function() {
    var options = { name: "x_ReportsTo", selectId: "fusersadd_x_ReportsTo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fusersadd.lists.ReportsTo?.lookupOptions.length) {
        options.data = { id: "x_ReportsTo", form: "fusersadd" };
    } else {
        options.ajax = { id: "x_ReportsTo", form: "fusersadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
loadjs.ready(['fusersadd', 'jqueryinputmask'], function() {
	...
	ew.createjQueryInputMask("fusersadd", "x_ReportsTo", jQuery.extend(true, "", options));
});
</script>
</span>
<?php } ?>
...
loadjs.ready(['fusersadd', 'jqueryinputmask'], function() {
	...
	ew.createjQueryInputMask("fusersadd", "x_CreatedAt", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
<span id="el_users_CreatedBy">
    <select
        ...
        data-select2-id="fusersadd_x_CreatedBy"
        <?php } ?>
        data-table="users"
...
<script<?= Nonce() ?>>
loadjs.ready("fusersadd", function() {
    var options = { name: "x_CreatedBy", selectId: "fusersadd_x_CreatedBy" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fusersadd.lists.CreatedBy?.lookupOptions.length) {
        options.data = { id: "x_CreatedBy", form: "fusersadd" };
    } else {
        options.ajax = { id: "x_CreatedBy", form: "fusersadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumInputLength = ew.selectMinimumInputLength;
