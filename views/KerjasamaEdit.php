...
<form name="fkerjasamaedit" id="fkerjasamaedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script<?= Nonce() ?>>
...
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fkerjasamaedit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["kerjasama_id", [fields.kerjasama_id.visible && fields.kerjasama_id.required ? ew.Validators.required(fields.kerjasama_id.caption) : null], fields.kerjasama_id.isInvalid],
...
<?php if ($Page->kerjasama_id->Visible) { // kerjasama_id ?>
    <div id="r_kerjasama_id"<?= $Page->kerjasama_id->rowAttributes() ?>>
        <label id="elh_kerjasama_kerjasama_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->kerjasama_id->caption() ?><?= $Page->kerjasama_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->kerjasama_id->cellAttributes() ?>>
<span id="el_kerjasama_kerjasama_id">
<span<?= $Page->kerjasama_id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->kerjasama_id->getDisplayValue($Page->kerjasama_id->getEditValue()))) ?>"></span>
<input type="hidden" data-table="kerjasama" data-field="x_kerjasama_id" data-hidden="1" name="x_kerjasama_id" id="x_kerjasama_id" value="<?= HtmlEncode($Page->kerjasama_id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<script<?= Nonce() ?>>
loadjs.ready(['fkerjasamaedit', 'jqueryinputmask'], function() {
	options = {
		'alias': 'numeric',
		'autoUnmask': true,
		'jitMasking': false,
	...
	ew.createjQueryInputMask("fkerjasamaedit", "x_mitra_id", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
<script<?= Nonce() ?>>
loadjs.ready(["fkerjasamaedit", "datetimepicker"], function () {
...
loadjs.ready(['fkerjasamaedit', 'jqueryinputmask'], function() {
	...
	ew.createjQueryInputMask("fkerjasamaedit", "x_tanggal_mulai", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
loadjs.ready(['fkerjasamaedit', 'jqueryinputmask'], function() {
	...
	ew.createjQueryInputMask("fkerjasamaedit", "x_tanggal_berakhir", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fkerjasamaedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fkerjasamaedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
...
<script>
loadjs.ready("head", function() { $("#fkerjasamaedit:first *:input[type!=hidden]:first").focus(),$("input").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("select").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("radio").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()})});
</script>
<?php } ?>
<script<?= Nonce() ?>>
