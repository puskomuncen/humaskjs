		  ...
		  if ( $value > 0 ) {
			  ...
			  if ($vParent != "") {		   
				...
				if ($value == "") {
				 ...
				 $rsb = ExecuteStatement("CALL ".MS_BREADCRUMB_LINKS_MOVE_SP."('".$CurrentRoot."','".$NewRoot."')");
				  if (!empty($rsb)) {  
...
<form name="fmovebreadcrumblink" id="fmovebreadcrumblink" method="get" action="<?php echo CurrentPageName() ?>" class="ms-breadcrumblinks-form form-horizontal ew-form ew-add-form">
	<div class="form-group row">
		<label class="col-sm-12 control-label ew-label" for="CurrentRoot"><?php echo $Language->phrase("Current Page Title"); ?> <?php echo $Language->phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-12">
		<?php
	...
	<div class="form-group row">
		<label class="col-sm-12 control-label ew-label" for="NewRoot"><?php echo $Language->phrase("New Root (Page Title)"); ?> <?php echo $Language->phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-12">
		<?php
		$sSql = "SELECT Page_Title FROM ".Config("MS_MASINO_BREADCRUMBLINKS_TABLE")." ORDER BY Page_Title ASC";
		$rs = ExecuteQuery($sSql);
...
<?php } ?>
<script<?= Nonce() ?>>
loadjs.ready(["wrapper", "head", "swal"],function(){$('#btn-action').on('click',function(){var $ = jQuery; if(fmovebreadcrumblink.validateFields()==true){ew.prompt({html: ew.language.phrase("MessageEditConfirm"),icon:'question',showCancelButton:true},result=>{if(result) $("#fmovebreadcrumblink").submit();});return false;}});});
</script>
