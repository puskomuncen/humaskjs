...
<form name="faddbreadcrumblink" id="faddbreadcrumblink" method="get" action="<?php echo CurrentPageName() ?>" class="ms-breadcrumblinks-form form-horizontal ew-form ew-add-form">
	<div class="form-group row">
		<label class="col-sm-12 control-label ew-label" for="pagetitleparent"><?php echo $Language->phrase("Page Title Parent"); ?> <?php echo $Language->phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-12">
		<?php
		...
		if ($cntRec > 0) {
		  ...
		  while ($row = $rs->fetchAssociative()) { // loop
	...
	<div class="form-group row">
		<label class="col-sm-12 control-label ew-label" for="pagetitleparent"><?php echo $Language->phrase("Page Title"); ?> <?php echo $Language->phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-12">
	...
	<div class="form-group row">
		<label class="col-sm-12 control-label ew-label" for="pagetitleparent"><?php echo $Language->phrase("Page URL"); ?></label>
		<div class="col-sm-12">
...
<?php } ?>
<script<?= Nonce() ?>>
loadjs.ready(["wrapper", "head", "swal"],function(){$('#btn-action').on('click',function(){var $ = jQuery; if(faddbreadcrumblink.validateFields()==true){ew.prompt({html: ew.language.phrase("MessageAddConfirm"),icon:'question',showCancelButton:true},result=>{if(result) $("#faddbreadcrumblink").submit();});return false;}});});
</script>
