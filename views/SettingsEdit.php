...
<form name="fsettingsedit" id="fsettingsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script<?= Nonce() ?>>
...
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        ...
        // Add fields
        .setFields([
            ["Option_ID", [fields.Option_ID.visible && fields.Option_ID.required ? ew.Validators.required(fields.Option_ID.caption) : null], fields.Option_ID.isInvalid],
...
<?php if ($Page->Option_ID->Visible) { // Option_ID ?>
    <div id="r_Option_ID"<?= $Page->Option_ID->rowAttributes() ?>>
        <label id="elh_settings_Option_ID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Option_ID->caption() ?><?= $Page->Option_ID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->Option_ID->cellAttributes() ?>>
<span id="el_settings_Option_ID">
<span<?= $Page->Option_ID->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->Option_ID->getDisplayValue($Page->Option_ID->getEditValue()))) ?>"></span>
<input type="hidden" data-table="settings" data-field="x_Option_ID" data-hidden="1" name="x_Option_ID" id="x_Option_ID" value="<?= HtmlEncode($Page->Option_ID->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<script<?= Nonce() ?>>
loadjs.ready(["fsettingsedit", "datetimepicker"], function () {
...
<?php } ?>
<script<?= Nonce() ?>>
loadjs.ready(['fsettingsedit', 'jqueryinputmask'], function() {
	options = {
		'jitMasking': false,
		'removeMaskOnSubmit': true
	};
	ew.createjQueryInputMask("fsettingsedit", "x_Maintenance_Finish_DateTime", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsettingsedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsettingsedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
...
<script>
loadjs.ready("head", function() { $("#fsettingsedit:first *:input[type!=hidden]:first").focus(),$("input").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("select").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("radio").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()})});
</script>
<?php } ?>
<script<?= Nonce() ?>>
