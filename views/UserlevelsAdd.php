...
<script<?= Nonce() ?>>
loadjs.ready(['fuserlevelsadd', 'jqueryinputmask'], function() {
	options = {
		'alias': 'numeric',
		'autoUnmask': true,
		'jitMasking': false,
	...
	ew.createjQueryInputMask("fuserlevelsadd", "x_ID", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
<span id="el_userlevels_Hierarchy">
    <select
        ...
        data-select2-id="fuserlevelsadd_x_Hierarchy[]"
        <?php } ?>
...
loadjs.ready("fuserlevelsadd", function() {
    var options = { name: "x_Hierarchy[]", selectId: "fuserlevelsadd_x_Hierarchy[]" },
    ...
    if (fuserlevelsadd.lists.Hierarchy?.lookupOptions.length) {
        options.data = { id: "x_Hierarchy[]", form: "fuserlevelsadd" };
    } else {
        options.ajax = { id: "x_Hierarchy[]", form: "fuserlevelsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_userlevels_Level_Origin">
    <select
        ...
        data-select2-id="fuserlevelsadd_x_Level_Origin"
        <?php } ?>
        data-table="userlevels"
...
<script<?= Nonce() ?>>
loadjs.ready("fuserlevelsadd", function() {
    var options = { name: "x_Level_Origin", selectId: "fuserlevelsadd_x_Level_Origin" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fuserlevelsadd.lists.Level_Origin?.lookupOptions.length) {
        options.data = { id: "x_Level_Origin", form: "fuserlevelsadd" };
    } else {
        options.ajax = { id: "x_Level_Origin", form: "fuserlevelsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumInputLength = ew.selectMinimumInputLength;
