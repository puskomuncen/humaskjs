...
<form name="fannouncementadd" id="fannouncementadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<script<?= Nonce() ?>>
loadjs.ready(["fannouncementadd", "editor"], function() {
    ew.createEditor("fannouncementadd", "x_Message", 50, 5, <?= $Page->Message->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
...
<script<?= Nonce() ?>>
loadjs.ready(["fannouncementadd", "datetimepicker"], function () {
...
<?php } ?>
<script<?= Nonce() ?>>
loadjs.ready(['fannouncementadd', 'jqueryinputmask'], function() {
	options = {
		'jitMasking': false,
		'removeMaskOnSubmit': true
	};
	ew.createjQueryInputMask("fannouncementadd", "x_Date_LastUpdate", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
<span id="el_announcement__Language">
    <select
        ...
        data-select2-id="fannouncementadd_x__Language"
        <?php } ?>
...
<?php if (!$Page->_Language->IsNativeSelect) { ?>
<script<?= Nonce() ?>>
loadjs.ready("fannouncementadd", function() {
    var options = { name: "x__Language", selectId: "fannouncementadd_x__Language" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fannouncementadd.lists._Language?.lookupOptions.length) {
        options.data = { id: "x__Language", form: "fannouncementadd" };
    } else {
        options.ajax = { id: "x__Language", form: "fannouncementadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
loadjs.ready(['fannouncementadd', 'jqueryinputmask'], function() {
	...
	ew.createjQueryInputMask("fannouncementadd", "x_Date_Start", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
loadjs.ready(['fannouncementadd', 'jqueryinputmask'], function() {
	...
	ew.createjQueryInputMask("fannouncementadd", "x_Date_End", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
loadjs.ready(['fannouncementadd', 'jqueryinputmask'], function() {
	...
	ew.createjQueryInputMask("fannouncementadd", "x_Date_Created", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
<span id="el_announcement_Created_By">
    <select
        ...
        data-select2-id="fannouncementadd_x_Created_By"
        <?php } ?>
        data-table="announcement"
...
<script<?= Nonce() ?>>
loadjs.ready("fannouncementadd", function() {
    var options = { name: "x_Created_By", selectId: "fannouncementadd_x_Created_By" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fannouncementadd.lists.Created_By?.lookupOptions.length) {
        options.data = { id: "x_Created_By", form: "fannouncementadd" };
    } else {
        options.ajax = { id: "x_Created_By", form: "fannouncementadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumInputLength = ew.selectMinimumInputLength;
...
<span id="el_announcement_Translated_ID">
    <select
        ...
        data-select2-id="fannouncementadd_x_Translated_ID"
        <?php } ?>
        data-table="announcement"
...
<script<?= Nonce() ?>>
loadjs.ready("fannouncementadd", function() {
    var options = { name: "x_Translated_ID", selectId: "fannouncementadd_x_Translated_ID" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fannouncementadd.lists.Translated_ID?.lookupOptions.length) {
        options.data = { id: "x_Translated_ID", form: "fannouncementadd" };
    } else {
        options.ajax = { id: "x_Translated_ID", form: "fannouncementadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumInputLength = ew.selectMinimumInputLength;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fannouncementadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fannouncementadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
...
<script>
loadjs.ready("head", function() { $("#fannouncementadd:first *:input[type!=hidden]:first").focus(),$("input").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("select").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("radio").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()})});
</script>
<?php } ?>
<script<?= Nonce() ?>>
