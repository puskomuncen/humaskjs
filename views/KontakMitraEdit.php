...
<form name="fkontak_mitraedit" id="fkontak_mitraedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script<?= Nonce() ?>>
var currentTable = <?= json_encode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { kontak_mitra: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fkontak_mitraedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fkontak_mitraedit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["kontak_id", [fields.kontak_id.visible && fields.kontak_id.required ? ew.Validators.required(fields.kontak_id.caption) : null], fields.kontak_id.isInvalid],
...
<?php if ($Page->kontak_id->Visible) { // kontak_id ?>
    <div id="r_kontak_id"<?= $Page->kontak_id->rowAttributes() ?>>
        <label id="elh_kontak_mitra_kontak_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->kontak_id->caption() ?><?= $Page->kontak_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->kontak_id->cellAttributes() ?>>
<span id="el_kontak_mitra_kontak_id">
<span<?= $Page->kontak_id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->kontak_id->getDisplayValue($Page->kontak_id->getEditValue()))) ?>"></span>
<input type="hidden" data-table="kontak_mitra" data-field="x_kontak_id" data-hidden="1" name="x_kontak_id" id="x_kontak_id" value="<?= HtmlEncode($Page->kontak_id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<script<?= Nonce() ?>>
loadjs.ready(['fkontak_mitraedit', 'jqueryinputmask'], function() {
	options = {
		'alias': 'numeric',
		'autoUnmask': true,
		'jitMasking': false,
	...
	ew.createjQueryInputMask("fkontak_mitraedit", "x_mitra_id", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fkontak_mitraedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fkontak_mitraedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
...
<script>
loadjs.ready("head", function() { $("#fkontak_mitraedit:first *:input[type!=hidden]:first").focus(),$("input").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("select").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("radio").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()})});
</script>
<?php } ?>
<script<?= Nonce() ?>>
