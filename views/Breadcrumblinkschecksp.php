	...
	function GetBreadcrumbLinks($pt) {
		...
		if ($pt != "") {
			...
			while ($row = $rs->fetchAssociative()) {
				if ($i < $rs->rowCount()) {
					if ($i==1) {
						$nav .= "<li><a href='home' title='".$Language->phrase("Home")."' alt='".$Language->phrase("Home")."' class='ew-home' /><span class='fa fa-home ew-icon'></span></a>&nbsp;<span class=\"divider\">".MS_BREADCRUMBLINKS_DIVIDER."</span>&nbsp;</li>";
					} else {
						$nav .= "<li><a href='". $row["Page_URL"]."'>".$Language->phrase($row["Page_Title"])."</a>&nbsp;<span class=\"divider\">".MS_BREADCRUMBLINKS_DIVIDER."</span>&nbsp;</li>";
					}
				} else {
	...
	if ( Get("PageTitle") ) {
		...
		if ( $value > 0 ) {
			...
			if ($pt == "") {
				...
				if ($recCount > 0) {
					if ($pt != $Language->phrase($pt)) {
						CurrentPage()->setMessage("Value in database: " . $pt);
						CurrentPage()->setMessage("Value in .xml file: " . $Language->phrase($pt));
						CurrentPage()->setWarningMessage($Language->phrase("CheckBreadcrumbLinksNotDefinedInXML"));
					}
				} else {
...
<?php } ?>
	<div class="form-group row">
		<label class="col-sm-12 control-label ew-label" for="pagetitle"><?php echo $Language->phrase("Page Title"); ?> <?php echo $Language->phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-12">
		<?php
