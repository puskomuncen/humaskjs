...
<?php if ($Page->Help_ID->Visible) { // Help_ID ?>
    <div id="r_Help_ID"<?= $Page->Help_ID->rowAttributes() ?>>
        <label id="elh_help_Help_ID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Help_ID->caption() ?><?= $Page->Help_ID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->Help_ID->cellAttributes() ?>>
<span id="el_help_Help_ID">
<span<?= $Page->Help_ID->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->Help_ID->getDisplayValue($Page->Help_ID->getEditValue()))) ?>"></span>
<input type="hidden" data-table="help" data-field="x_Help_ID" data-hidden="1" name="x_Help_ID" id="x_Help_ID" value="<?= HtmlEncode($Page->Help_ID->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_help__Language">
    <select
        ...
        data-select2-id="fhelpedit_x__Language"
        <?php } ?>
        data-table="help"
...
loadjs.ready("fhelpedit", function() {
    var options = { name: "x__Language", selectId: "fhelpedit_x__Language" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fhelpedit.lists._Language?.lookupOptions.length) {
        options.data = { id: "x__Language", form: "fhelpedit" };
    } else {
        options.ajax = { id: "x__Language", form: "fhelpedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
loadjs.ready(["fhelpedit", "editor"], function() {
    ew.createEditor("fhelpedit", "x_Description", 50, 5, <?= $Page->Description->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
...
<span id="el_help_Category">
    <select
        ...
        data-select2-id="fhelpedit_x_Category"
        <?php } ?>
        data-table="help"
...
<script<?= Nonce() ?>>
loadjs.ready("fhelpedit", function() {
    var options = { name: "x_Category", selectId: "fhelpedit_x_Category" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fhelpedit.lists.Category?.lookupOptions.length) {
        options.data = { id: "x_Category", form: "fhelpedit" };
    } else {
        options.ajax = { id: "x_Category", form: "fhelpedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumInputLength = ew.selectMinimumInputLength;
...
<script<?= Nonce() ?>>
loadjs.ready(['fhelpedit', 'jqueryinputmask'], function() {
	options = {
		'alias': 'numeric',
		'autoUnmask': true,
		'jitMasking': false,
	...
	ew.createjQueryInputMask("fhelpedit", "x_Order", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
<span id="el_help_Updated_By">
    <select
        ...
        data-select2-id="fhelpedit_x_Updated_By"
        <?php } ?>
        data-table="help"
...
<script<?= Nonce() ?>>
loadjs.ready("fhelpedit", function() {
    var options = { name: "x_Updated_By", selectId: "fhelpedit_x_Updated_By" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fhelpedit.lists.Updated_By?.lookupOptions.length) {
        options.data = { id: "x_Updated_By", form: "fhelpedit" };
    } else {
        options.ajax = { id: "x_Updated_By", form: "fhelpedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumInputLength = ew.selectMinimumInputLength;
...
<script<?= Nonce() ?>>
loadjs.ready(["fhelpedit", "datetimepicker"], function () {
...
loadjs.ready(['fhelpedit', 'jqueryinputmask'], function() {
	...
	ew.createjQueryInputMask("fhelpedit", "x_Last_Updated", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
<script>
loadjs.ready("head", function() { $("#fhelpedit:first *:input[type!=hidden]:first").focus(),$("input").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("select").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("radio").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()})});
</script>
<?php } ?>
<script<?= Nonce() ?>>
