	...
	if ( $PageTitle != "" ) {	
	  ...
	  if ( $value > 0 ) {
			...
			if ($pt == "") {
			 ...
			 $rsb = ExecuteStatement("CALL ".MS_BREADCRUMB_LINKS_DELETE_SP."('".$PageTitle."')");
			  if (!empty($rsb)) {  
...
?>
loadjs.ready(["wrapper", "head", "swal"],function(){$('#btn-action').on('click',function(){var $ = jQuery; if(fdeletebreadcrumblink.validateFields()==true){ew.prompt({html: "<?php echo $warning_delete; ?>",icon:'question',showCancelButton:true},result=>{if(result) $("#fdeletebreadcrumblink").submit();});return false;}});});
</script>
