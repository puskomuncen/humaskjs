...
<form name="fuserlevelpermissionsedit" id="fuserlevelpermissionsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script<?= Nonce() ?>>
...
<script<?= Nonce() ?>>
loadjs.ready(['fuserlevelpermissionsedit', 'jqueryinputmask'], function() {
	options = {
		'alias': 'numeric',
		'autoUnmask': true,
		'jitMasking': false,
	...
	ew.createjQueryInputMask("fuserlevelpermissionsedit", "x_UserLevelID", jQuery.extend(true, "", options));
});
</script>
...
loadjs.ready(['fuserlevelpermissionsedit', 'jqueryinputmask'], function() {
	...
	ew.createjQueryInputMask("fuserlevelpermissionsedit", "x__Permission", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
